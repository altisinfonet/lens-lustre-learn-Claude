 ...
 * the edge really returns is P15 and is a fetch against the running site, not
 ...
 * amount of counting code points can tell them apart. When P12 emits one chunk
...
export function classifyJsChunk(relPath, src) {
  ...
  //     This is the shape P12 will emit (translations.ta-<hash>.js) and it is
...
export function gitProvenance(root) {
  /* Review F-3, 2026-09-02. `.git` is a DIRECTORY in a plain clone and a FILE
   * in a worktree ("gitdir: <path>"). The first version read `.git/HEAD` as a
   * path and, in a worktree, threw and swallowed — the run record then said
   * { commit: null, branch: null } from a checkout that had a commit. Still
   * built-ins only: the pointer file is one line, and a worktree's branch refs
   * live in the COMMON dir (`<gitdir>/commondir`), not under the worktree.
   * When provenance cannot be read the record says so — `status: "unknown"`
   * with a reason — because a null that means "could not read" must never look
   * like a null that means "not a repository". */
  const read = (p) => fs.readFileSync(p, "utf8").trim();
  try {
    let dotGit = path.join(root, ".git");
    const st = fs.statSync(dotGit);
    if (st.isFile()) {
      const m = read(dotGit).match(/^gitdir:\s*(.+)$/m);
      if (!m) return { commit: null, branch: null, status: "unknown", reason: ".git file carries no gitdir: pointer" };
      dotGit = path.resolve(root, m[1]);
    }
    let common = dotGit;
    const commondir = path.join(dotGit, "commondir");
    if (fs.existsSync(commondir)) common = path.resolve(dotGit, read(commondir));

    const head = read(path.join(dotGit, "HEAD"));
    const m = head.match(/^ref:\s*(.+)$/);
    if (!m) return { commit: head, branch: null };
    const ref = m[1];
    const branch = ref.replace(/^refs\/heads\//, "");
    for (const dir of [dotGit, common]) {
      const refFile = path.join(dir, ref);
      if (fs.existsSync(refFile)) return { commit: read(refFile), branch };
    }
    for (const dir of [common, dotGit]) {
    ...
    return { commit: null, branch, status: "unknown", reason: `ref ${ref} not found under ${dotGit} or ${common}` };
  } catch (err) {
    return { commit: null, branch: null, status: "unknown", reason: `no readable .git at ${root}: ${err.message}` };
  }
}
      ...
      languageRecords.push(
        stamp({
          ...
          blockedReason: single
            ? null
            : `${d.languages.length} languages share ${d.path}; a per-language byte size does not exist in this build. P12 (one chunk per language) is what makes it measurable.`,
          estimate: {
  ...
  const records = [
    stamp({
      ...
      compression: {
        ...
        meaning: "A floor for what a CDN can deliver, computed here. NOT a measurement of what the edge actually returns — that is P15, and it is a fetch, not arithmetic.",
      },
