...
--
-- Run top-to-bottom on a fresh Supabase project's public schema.
-- Idempotent: safe to re-run (IF NOT EXISTS / CREATE OR REPLACE throughout).
--
-- REQUIRED EXTENSIONS on the target project before running this script:
--   pgcrypto, uuid-ossp, pg_net, pg_trgm, pgmq, pg_cron, pg_stat_statements,
--   supabase_vault, plpgsql_check
-- (pg_trgm backs two GIN trigram indexes; pgmq backs several queue
...
-- Includes standalone sequences AND the two column-owned ("serial-style")
-- sequences whose owning table's column default calls nextval(...) directly
-- (media_repair_audit.id, push_delivery_log.id) -- these must exist before
-- their tables are created, so they are listed here even though they are
-- technically owned sequences rather than free-standing ones.

CREATE SEQUENCE IF NOT EXISTS public.competition_order_no_seq;

CREATE SEQUENCE IF NOT EXISTS public.media_repair_audit_id_seq;

CREATE SEQUENCE IF NOT EXISTS public.push_delivery_log_id_seq;

-- =====================================================================
-- ===== TABLES =====
-- =====================================================================

CREATE TABLE IF NOT EXISTS public._v3_preflight_snapshot_competition_entries (
  id uuid,
  competition_id uuid,
  user_id uuid,
  title text,
...
CREATE TABLE IF NOT EXISTS public._v3_preflight_snapshot_judge_decisions (
  id uuid,
  entry_id uuid,
  judge_id uuid,
...
CREATE TABLE IF NOT EXISTS public._v3_preflight_snapshot_judge_tag_assignments (
  id uuid,
  entry_id uuid,
  tag_id uuid,
  judge_id uuid,
...
CREATE TABLE IF NOT EXISTS public._v3_preflight_snapshot_judging_tags (
  id uuid,
  label text,
  color text,
  sort_order integer,
  ...
  icon text,
  image_url text,
  is_quality_tag boolean,
  visible_in_round integer[],
  is_system boolean,
  is_visible boolean,
  _snapshot_at timestamp with time zone
);
...
CREATE TABLE IF NOT EXISTS public._v3_quarantine_tag_assignments (
  ...
  quarantine_reason text NOT NULL,
  quarantine_phase text DEFAULT 'v3_step_0_3'::text NOT NULL,
  quarantined_at timestamp with time zone DEFAULT now() NOT NULL,
  raw_payload jsonb
);
...
CREATE TABLE IF NOT EXISTS public.activity_logs (
  ...
  action_category text DEFAULT 'auth'::text NOT NULL,
  description text,
...
CREATE TABLE IF NOT EXISTS public.ad_impressions (
  ...
  placement text NOT NULL,
  event_type text DEFAULT 'impression'::text NOT NULL,
  device text DEFAULT 'desktop'::text NOT NULL,
  country text,
  ad_source text DEFAULT 'internal'::text NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.admin_notifications (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  type text DEFAULT 'support_ticket'::text NOT NULL,
  title text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.badge_definitions (
  ...
  badge_class text DEFAULT 'bg-amber-500/15 text-amber-600 border-amber-500/30'::text NOT NULL,
  ribbon_class text DEFAULT 'bg-gradient-to-r from-amber-500 to-yellow-400 text-white shadow-amber-500/30'::text NOT NULL,
  sort_order integer DEFAULT 0 NOT NULL,
  is_active boolean DEFAULT true NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.blocked_keywords (
  ...
  category text DEFAULT 'profanity'::text NOT NULL,
  severity text DEFAULT 'auto_hide'::text NOT NULL,
  is_active boolean DEFAULT true NOT NULL,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
...
CREATE TABLE IF NOT EXISTS public.chat_questions (
  ...
  promoted_to_faq boolean DEFAULT false NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.competition_entries (
  ...
  exif_data jsonb,
  current_round text,
  certificate_ready boolean DEFAULT false NOT NULL,
  photo_thumbnails text[],
  photo_meta jsonb DEFAULT '[]'::jsonb NOT NULL,
  is_ai_advisory boolean DEFAULT true NOT NULL,
  progression_decision text,
  stage_key text,
...
CREATE TABLE IF NOT EXISTS public.competition_judging_tags (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  competition_id uuid NOT NULL,
  tag_id uuid NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.competition_orders (
  ...
  competition_id uuid NOT NULL,
  entry_id uuid,
  order_type text DEFAULT 'photo_competition'::text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.competition_payment_details (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  competition_id uuid NOT NULL,
  paypal_email text,
  bank_details text,
...
CREATE TABLE IF NOT EXISTS public.competitions (
  ...
  max_photos_per_entry integer DEFAULT 5,
  starts_at timestamp with time zone NOT NULL,
  ends_at timestamp with time zone NOT NULL,
  created_by uuid NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
  ai_images_allowed boolean DEFAULT true NOT NULL,
  slug text DEFAULT ''::text,
  phase text DEFAULT 'submission_open'::text NOT NULL,
  current_round text,
  judge_assignment_mode text DEFAULT 'shared'::text NOT NULL,
  voting_ends_at timestamp with time zone,
  judging_completed boolean DEFAULT false NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.contributor_engagement_daily (
  ...
  minutes_public integer DEFAULT 0 NOT NULL,
  minutes_internal integer DEFAULT 0 NOT NULL,
  minutes_credited integer DEFAULT 0 NOT NULL,
  minutes_interacted integer DEFAULT 0 NOT NULL,
  minutes_carried integer DEFAULT 0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.courses (
  ...
  admin_students integer DEFAULT 0,
  admin_rating numeric DEFAULT 0,
  admin_rating_count integer DEFAULT 0,
  reviews_enabled boolean DEFAULT false,
  notification_sent boolean DEFAULT false NOT NULL
);

CREATE TABLE IF NOT EXISTS public.custom_url_history (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  user_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.featured_artists (
  ...
  cover_image_url text,
  photo_gallery text[] DEFAULT '{}'::text[] NOT NULL,
  artist_name text,
  artist_bio text,
...
CREATE TABLE IF NOT EXISTS public.gift_credits (
  ...
  target_value text,
  auto_apply_future boolean DEFAULT false NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.held_result_notifications (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  competition_id uuid NOT NULL,
  round_number integer NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.image_comments (
  ...
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
  is_pinned boolean DEFAULT false NOT NULL,
  is_admin_seed boolean DEFAULT false NOT NULL,
  photo_index integer DEFAULT 0 NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.judge_entry_locks (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  entry_id uuid NOT NULL,
  judge_id uuid NOT NULL,
  photo_index integer DEFAULT 0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.judge_tag_assignments (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  entry_id uuid NOT NULL,
  tag_id uuid NOT NULL,
  judge_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.judging_config (
  ...
  competition_id uuid NOT NULL,
  round_number integer DEFAULT 1 NOT NULL,
  threshold numeric DEFAULT 0.5 NOT NULL,
  min_judges integer DEFAULT 2 NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.judging_preflight_log (
  ...
  ui_only_sample jsonb DEFAULT '[]'::jsonb NOT NULL,
  db_only_sample jsonb DEFAULT '[]'::jsonb NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.judging_tags (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  label text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.media_repair_audit (
  id bigint DEFAULT nextval('media_repair_audit_id_seq'::regclass) NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.member_activity_minutes (
  ...
  had_interaction boolean DEFAULT false NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.notification_preferences (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  user_id uuid NOT NULL,
  email_reactions boolean DEFAULT true NOT NULL,
  email_comments boolean DEFAULT true NOT NULL,
  email_friend_requests boolean DEFAULT true NOT NULL,
  email_new_followers boolean DEFAULT true NOT NULL,
  email_competition_updates boolean DEFAULT true NOT NULL,
  email_gift_credits boolean DEFAULT true NOT NULL,
  email_certificates boolean DEFAULT true NOT NULL,
  email_course_updates boolean DEFAULT true NOT NULL,
  inapp_reactions boolean DEFAULT true NOT NULL,
  inapp_comments boolean DEFAULT true NOT NULL,
  inapp_social boolean DEFAULT true NOT NULL,
  inapp_competitions boolean DEFAULT true NOT NULL,
  email_weekly_digest boolean DEFAULT true NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
  email_reengagement boolean DEFAULT true NOT NULL,
  push_enabled boolean DEFAULT true NOT NULL,
  push_reactions boolean DEFAULT true NOT NULL,
  ...
  push_competition_updates boolean DEFAULT true NOT NULL,
  push_new_posts boolean DEFAULT false NOT NULL,
  push_announcements boolean DEFAULT true NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.photo_albums (
  ...
  user_id uuid NOT NULL,
  name text NOT NULL,
  album_type text DEFAULT 'custom'::text NOT NULL,
  cover_url text,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
);

CREATE TABLE IF NOT EXISTS public.photo_of_the_day (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  image_url text NOT NULL,
  title text NOT NULL,
  ...
  description text,
  is_active boolean DEFAULT true NOT NULL,
  featured_date date DEFAULT CURRENT_DATE NOT NULL,
  created_by uuid NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  active_from timestamp with time zone,
  active_until timestamp with time zone,
...
CREATE TABLE IF NOT EXISTS public.post_drafts (
  ...
  pending_tags jsonb DEFAULT '[]'::jsonb NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.post_tags (
  ...
  status post_tag_status DEFAULT 'pending'::post_tag_status NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.posts_dead_host_backup_20260812 (
  id uuid,
  image_urls text[],
  thumbnail_urls text[]
);
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  twitter_url text,
  youtube_url text,
  privacy_settings jsonb DEFAULT '{"bio": "public", "email": "only_me", "phone": "only_me", "avatar": "public", "whatsapp": "only_me", "interests": "public", "portfolio": "public", "city_country": "public", "social_links": "public"}'::jsonb NOT NULL,
  user_type text,
  ...
  custom_url_changed_at timestamp with time zone,
  indexing_disabled boolean DEFAULT false NOT NULL,
  reengagement_sends_count integer DEFAULT 0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.push_delivery_log (
  id bigint DEFAULT nextval('push_delivery_log_id_seq'::regclass) NOT NULL,
  user_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.raw_commitments (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  entry_id uuid NOT NULL,
  competition_id uuid NOT NULL,
  user_id uuid NOT NULL,
  ...
  raw_delivered_at timestamp with time zone,
  raw_file_url text,
...
CREATE TABLE IF NOT EXISTS public.role_display_config (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  role_key text NOT NULL,
  label text NOT NULL,
  icon text DEFAULT ''::text NOT NULL,
  pill_class text DEFAULT 'bg-muted text-muted-foreground border-border'::text NOT NULL,
  show_inline boolean DEFAULT true NOT NULL,
  sort_order integer DEFAULT 0 NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.scheduled_boosts (
  ...
  image_type text NOT NULL,
  reaction_type text DEFAULT 'like'::text NOT NULL,
  total_amount integer DEFAULT 10 NOT NULL,
  applied_amount integer DEFAULT 0 NOT NULL,
  increment_per_hour integer DEFAULT 1 NOT NULL,
  status text DEFAULT 'active'::text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.scheduled_posts (
  ...
  shifted_count integer DEFAULT 0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.user_notifications (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  user_id uuid NOT NULL,
  type text DEFAULT 'ticket_reply'::text NOT NULL,
  title text NOT NULL,
  message text NOT NULL,
  reference_id uuid,
  is_read boolean DEFAULT false NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.user_roles (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  user_id uuid NOT NULL,
  role text DEFAULT 'user'::app_role NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.v3_stage_catalog (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  stage_key text NOT NULL,
  round_number integer NOT NULL,
  ...
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL,
  cert_eligible boolean DEFAULT false NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.v3_tag_label_alias (
  ...
  round_number integer NOT NULL,
  canonical_stage_key text NOT NULL,
  notes text,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  updated_at timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.wallet_ledger_audit_log (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  op text NOT NULL,
  actor_user_id uuid,
  target_user_id uuid,
...
CREATE TABLE IF NOT EXISTS public.wallet_ledger_idempotency (
  op text NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.wallet_ledger_shadow_log (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  source_path text,
  op text NOT NULL,
  idempotency_key text,
...
CREATE TABLE IF NOT EXISTS public.wallet_ledger_v2_diff_log (
  ...
  live_wallet_transactions_total bigint DEFAULT 0 NOT NULL,
  shadow_log_total bigint DEFAULT 0 NOT NULL,
  matched bigint DEFAULT 0 NOT NULL,
  unmatched_live bigint DEFAULT 0 NOT NULL,
  unmatched_shadow bigint DEFAULT 0 NOT NULL,
  amount_mismatch bigint DEFAULT 0 NOT NULL,
  type_mismatch bigint DEFAULT 0 NOT NULL,
  user_mismatch bigint DEFAULT 0 NOT NULL,
  reference_mismatch bigint DEFAULT 0 NOT NULL,
  error_count bigint DEFAULT 0 NOT NULL,
  mismatch_count bigint DEFAULT 0 NOT NULL,
  latest_mismatch_at timestamp with time zone,
  safe_for_shadow_wiring boolean,
  wallets_checksum text,
  raw_report jsonb,
  alert_fired boolean DEFAULT false NOT NULL,
  notes text
);
...
CREATE UNIQUE INDEX IF NOT EXISTS activity_logs_one_row_per_sign_in ON public.activity_logs USING btree (user_id, ((metadata ->> 'sign_in_at'::text))) WHERE ((action_type = 'login'::text) AND (metadata ? 'sign_in_at'::text));
CREATE UNIQUE INDEX IF NOT EXISTS blocked_keywords_keyword_unique ON public.blocked_keywords USING btree (lower(keyword));
CREATE UNIQUE INDEX IF NOT EXISTS competition_judges_one_per_competition ON public.competition_judges USING btree (competition_id);
CREATE UNIQUE INDEX IF NOT EXISTS custom_url_history_active_unique ON public.custom_url_history USING btree (lower(custom_url)) WHERE (is_current = true);
CREATE INDEX IF NOT EXISTS idx_activity_logs_action_category ON public.activity_logs USING btree (action_category);
CREATE INDEX IF NOT EXISTS idx_activity_logs_created_at ON public.activity_logs USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_activity_logs_is_archived ON public.activity_logs USING btree (is_archived);
...
CREATE INDEX IF NOT EXISTS idx_ad_conversions_ad_id ON public.ad_conversions USING btree (ad_id);
CREATE INDEX IF NOT EXISTS idx_ad_conversions_created ON public.ad_conversions USING btree (created_at);
CREATE INDEX IF NOT EXISTS idx_ad_conversions_type ON public.ad_conversions USING btree (conversion_type);
CREATE INDEX IF NOT EXISTS idx_ad_creative_comments_creative ON public.ad_creative_comments USING btree (creative_id, created_at);
CREATE INDEX IF NOT EXISTS idx_ad_creative_comments_parent ON public.ad_creative_comments USING btree (parent_id) WHERE (parent_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_ad_creative_reactions_creative ON public.ad_creative_reactions USING btree (creative_id);
CREATE INDEX IF NOT EXISTS idx_ad_creative_shares_creative ON public.ad_creative_shares USING btree (creative_id);
CREATE INDEX IF NOT EXISTS idx_ad_creatives_zone_active ON public.ad_creatives USING btree (zone, sort_order, created_at) WHERE is_active;
CREATE INDEX IF NOT EXISTS idx_ad_impressions_ad_source_created ON public.ad_impressions USING btree (ad_source, created_at);
CREATE INDEX IF NOT EXISTS idx_ad_impressions_creative ON public.ad_impressions USING btree (creative_id, event_type) WHERE (creative_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_ad_impressions_event ON public.ad_impressions USING btree (event_type, created_at);
CREATE INDEX IF NOT EXISTS idx_ad_impressions_placement ON public.ad_impressions USING btree (placement, created_at);
CREATE INDEX IF NOT EXISTS idx_ad_impressions_slot ON public.ad_impressions USING btree (slot_id, created_at);
CREATE INDEX IF NOT EXISTS idx_admin_vote_adj_competition ON public.admin_vote_adjustments USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_admin_vote_adj_entry ON public.admin_vote_adjustments USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_admin_vote_adjustments_entry_photo ON public.admin_vote_adjustments USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_album_photos_album ON public.album_photos USING btree (album_id);
CREATE INDEX IF NOT EXISTS idx_album_photos_post_id ON public.album_photos USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_blocked_keywords_created_by ON public.blocked_keywords USING btree (created_by);
CREATE INDEX IF NOT EXISTS idx_ce_comp_created ON public.competition_entries USING btree (competition_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ce_comp_round ON public.competition_entries USING btree (competition_id, current_round);
CREATE INDEX IF NOT EXISTS idx_ce_public_placement_derived ON public.competition_entries USING btree (public_placement_derived);
CREATE INDEX IF NOT EXISTS idx_ce_public_round_derived ON public.competition_entries USING btree (public_round_derived);
CREATE INDEX IF NOT EXISTS idx_certificate_testimonials_certificate_id ON public.certificate_testimonials USING btree (certificate_id);
CREATE INDEX IF NOT EXISTS idx_certificates_is_revoked ON public.certificates USING btree (is_revoked) WHERE (is_revoked = true);
CREATE INDEX IF NOT EXISTS idx_certificates_issued_at_id_desc ON public.certificates USING btree (issued_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_chat_questions_ask_count ON public.chat_questions USING btree (ask_count DESC);
CREATE INDEX IF NOT EXISTS idx_chat_questions_fingerprint ON public.chat_questions USING btree (question_fingerprint);
CREATE INDEX IF NOT EXISTS idx_client_errors_code ON public.client_errors USING btree (code, created_at DESC) WHERE (code IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_client_errors_correlation ON public.client_errors USING btree (correlation_id) WHERE (correlation_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_client_errors_created ON public.client_errors USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_client_errors_kind ON public.client_errors USING btree (kind, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_client_errors_severity ON public.client_errors USING btree (severity, created_at DESC) WHERE (severity IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_client_errors_user_recent ON public.client_errors USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comment_reports_ad_comment ON public.comment_reports USING btree (ad_comment_id) WHERE (ad_comment_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_comment_reports_comment_id ON public.comment_reports USING btree (comment_id);
CREATE INDEX IF NOT EXISTS idx_comment_reports_post_comment_id ON public.comment_reports USING btree (post_comment_id);
CREATE INDEX IF NOT EXISTS idx_comment_reports_status ON public.comment_reports USING btree (status);
CREATE INDEX IF NOT EXISTS idx_comments_article_id ON public.comments USING btree (article_id);
CREATE INDEX IF NOT EXISTS idx_comments_entry_id ON public.comments USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_comments_parent_id ON public.comments USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_comp_entries_comp_status ON public.competition_entries USING btree (competition_id, status);
CREATE INDEX IF NOT EXISTS idx_comp_entries_user ON public.competition_entries USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_comp_votes_entry ON public.competition_votes USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_competition_entries_current_round_int ON public.competition_entries USING btree (current_round_int);
CREATE INDEX IF NOT EXISTS idx_competition_entries_keyset ON public.competition_entries USING btree (competition_id, created_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_competition_entries_photo_meta ON public.competition_entries USING gin (photo_meta);
CREATE INDEX IF NOT EXISTS idx_competition_entries_photo_meta_gin ON public.competition_entries USING gin (photo_meta jsonb_path_ops);
CREATE INDEX IF NOT EXISTS idx_competition_entries_progression_decision ON public.competition_entries USING btree (progression_decision);
CREATE INDEX IF NOT EXISTS idx_competition_entries_public_status_derived ON public.competition_entries USING btree (public_status_derived);
CREATE INDEX IF NOT EXISTS idx_competition_entries_stage_key ON public.competition_entries USING btree (stage_key);
CREATE INDEX IF NOT EXISTS idx_competition_judging_tags_tag_id ON public.competition_judging_tags USING btree (tag_id);
CREATE INDEX IF NOT EXISTS idx_competition_orders_competition ON public.competition_orders USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_competition_orders_entry_id ON public.competition_orders USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_competition_orders_status ON public.competition_orders USING btree (status);
CREATE INDEX IF NOT EXISTS idx_competition_orders_user_created ON public.competition_orders USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_competition_orders_wallet_txn_id ON public.competition_orders USING btree (wallet_txn_id);
CREATE INDEX IF NOT EXISTS idx_competition_votes_entry_photo ON public.competition_votes USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_contributor_engagement_daily_date ON public.contributor_engagement_daily USING btree (utc_date);
...
CREATE INDEX IF NOT EXISTS idx_courses_author_id ON public.courses USING btree (author_id);
CREATE INDEX IF NOT EXISTS idx_custom_url_history_lookup ON public.custom_url_history USING btree (lower(custom_url));
CREATE INDEX IF NOT EXISTS idx_custom_url_history_user_id ON public.custom_url_history USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_db_audit_logs_changed_by ON public.db_audit_logs USING btree (changed_by, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_db_audit_logs_table_created ON public.db_audit_logs USING btree (table_name, created_at DESC);

CREATE INDEX IF NOT EXISTS held_result_notifications_release_idx ON public.held_result_notifications USING btree (competition_id, round_number);
...
CREATE INDEX IF NOT EXISTS idx_esc_avg ON public.entry_score_cache USING btree (avg_score DESC);
CREATE INDEX IF NOT EXISTS idx_featured_artists_author_profile_id ON public.featured_artists USING btree (author_profile_id);
CREATE INDEX IF NOT EXISTS idx_feed_events_author ON public.feed_events USING btree (author_id);
CREATE INDEX IF NOT EXISTS idx_feed_events_created ON public.feed_events USING btree (created_at);
CREATE INDEX IF NOT EXISTS idx_feed_events_post ON public.feed_events USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_feed_events_post_id ON public.feed_events USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_feed_events_user_author ON public.feed_events USING btree (user_id, author_id);
CREATE INDEX IF NOT EXISTS idx_feed_events_user_created ON public.feed_events USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_feed_events_user_post ON public.feed_events USING btree (user_id, post_id);
...
CREATE INDEX IF NOT EXISTS idx_friendships_addressee_status ON public.friendships USING btree (addressee_id, status);
CREATE INDEX IF NOT EXISTS idx_friendships_requester_status ON public.friendships USING btree (requester_id, status);
CREATE INDEX IF NOT EXISTS idx_gift_announcements_gift_credit_id ON public.gift_announcements USING btree (gift_credit_id);
CREATE INDEX IF NOT EXISTS idx_hashtags_rank ON public.hashtags USING btree (unique_user_count DESC, post_count DESC, last_used_at DESC);
CREATE INDEX IF NOT EXISTS idx_hashtags_tag_prefix ON public.hashtags USING btree (tag text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_highlight_items_highlight_id ON public.highlight_items USING btree (highlight_id);
CREATE INDEX IF NOT EXISTS idx_unsubscribe_tokens_token ON public.email_unsubscribe_tokens USING btree (token);
CREATE UNIQUE INDEX IF NOT EXISTS uq_friendships_pair_normalized ON public.friendships USING btree (LEAST(requester_id, addressee_id), GREATEST(requester_id, addressee_id));

CREATE INDEX IF NOT EXISTS idx_image_comments_image_photo ON public.image_comments USING btree (image_type, image_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_image_comments_lookup ON public.image_comments USING btree (image_type, image_id);
CREATE INDEX IF NOT EXISTS idx_image_comments_parent_id ON public.image_comments USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_image_reactions_image_photo ON public.image_reactions USING btree (image_type, image_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_image_reactions_lookup ON public.image_reactions USING btree (image_type, image_id);
CREATE INDEX IF NOT EXISTS idx_jc_entry ON public.judge_comments USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_jd_entry_round ON public.judge_decisions USING btree (entry_id, round_number);
CREATE INDEX IF NOT EXISTS idx_jd_judge_round ON public.judge_decisions USING btree (judge_id, round_number);
CREATE INDEX IF NOT EXISTS idx_jea_comp_judge ON public.judge_entry_assignments USING btree (competition_id, judge_id);
CREATE INDEX IF NOT EXISTS idx_journal_articles_author_id ON public.journal_articles USING btree (author_id);
CREATE INDEX IF NOT EXISTS idx_js_entry_judge ON public.judge_scores USING btree (entry_id, judge_id);
CREATE INDEX IF NOT EXISTS idx_js_judge ON public.judge_scores USING btree (judge_id);
CREATE INDEX IF NOT EXISTS idx_jta_entry ON public.judge_tag_assignments USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_jta_entry_judge ON public.judge_tag_assignments USING btree (entry_id, judge_id);
CREATE INDEX IF NOT EXISTS idx_judge_activity_logs_competition ON public.judge_activity_logs USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_judge_activity_logs_entry ON public.judge_activity_logs USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_judge_activity_logs_judge ON public.judge_activity_logs USING btree (judge_id);
CREATE INDEX IF NOT EXISTS idx_judge_award_tags_tag_id ON public.judge_award_tags USING btree (tag_id);
CREATE INDEX IF NOT EXISTS idx_judge_comments_entry_photo_judge ON public.judge_comments USING btree (entry_id, photo_index, judge_id);
CREATE INDEX IF NOT EXISTS idx_judge_comments_round_id ON public.judge_comments USING btree (round_id);
CREATE INDEX IF NOT EXISTS idx_judge_decisions_entry_photo ON public.judge_decisions USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_judge_decisions_stage_key ON public.judge_decisions USING btree (entry_id, judge_id, round_number, photo_index, stage_key);
CREATE INDEX IF NOT EXISTS idx_judge_entry_assignments_entry ON public.judge_entry_assignments USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_judge_entry_assignments_judge ON public.judge_entry_assignments USING btree (judge_id, competition_id);
CREATE INDEX IF NOT EXISTS idx_judge_scores_entry ON public.judge_scores USING btree (entry_id);
CREATE INDEX IF NOT EXISTS idx_judge_scores_entry_photo ON public.judge_scores USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_judge_scores_entry_round_judge_photo ON public.judge_scores USING btree (entry_id, round_number, judge_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_judge_sessions_competition_id ON public.judge_sessions USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_judge_sessions_last_entry_id ON public.judge_sessions USING btree (last_entry_id);
CREATE INDEX IF NOT EXISTS idx_judge_sessions_round_id ON public.judge_sessions USING btree (round_id);
CREATE INDEX IF NOT EXISTS idx_judge_tag_assignments_entry_photo ON public.judge_tag_assignments USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_judge_tag_assignments_tag_id ON public.judge_tag_assignments USING btree (tag_id);
CREATE INDEX IF NOT EXISTS idx_judging_preflight_log_comp_round ON public.judging_preflight_log USING btree (competition_id, round_number, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_judging_preflight_log_drift ON public.judging_preflight_log USING btree (drift_detected, created_at DESC) WHERE (drift_detected = true);
...
CREATE INDEX IF NOT EXISTS idx_member_activity_minutes_bucket ON public.member_activity_minutes USING btree (minute_bucket);
CREATE UNIQUE INDEX IF NOT EXISTS idx_one_active_round_per_competition ON public.judging_rounds USING btree (competition_id) WHERE (status = 'active'::text);
CREATE INDEX IF NOT EXISTS judge_award_tags_lookup_idx ON public.judge_award_tags USING btree (entry_id, judge_id, photo_index);
CREATE INDEX IF NOT EXISTS judge_award_tags_stage_idx ON public.judge_award_tags USING btree (entry_id, photo_index, stage_key);
CREATE UNIQUE INDEX IF NOT EXISTS judge_decisions_unique_per_judge_round_photo ON public.judge_decisions USING btree (entry_id, judge_id, round_number, photo_index);
CREATE UNIQUE INDEX IF NOT EXISTS media_objects_owner_content ON public.media_objects USING btree (owner_id, sha256);
CREATE INDEX IF NOT EXISTS media_objects_state_created ON public.media_objects USING btree (state, created_at) WHERE (state <> 'ready'::text);

CREATE INDEX IF NOT EXISTS idx_photo_albums_type ON public.photo_albums USING btree (user_id, album_type);
CREATE INDEX IF NOT EXISTS idx_photo_albums_user ON public.photo_albums USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_post_comments_parent_id ON public.post_comments USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_post_comments_post_created ON public.post_comments USING btree (post_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_post_comments_post_id ON public.post_comments USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_post_comments_user_created ON public.post_comments USING btree (user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_post_drafts_sweep ON public.post_drafts USING btree (updated_at) WHERE (expiring_at IS NULL);
CREATE INDEX IF NOT EXISTS idx_post_drafts_user ON public.post_drafts USING btree (user_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_post_hashtags_author ON public.post_hashtags USING btree (hashtag_id, author_id);
CREATE INDEX IF NOT EXISTS idx_post_hashtags_hashtag ON public.post_hashtags USING btree (hashtag_id);
CREATE INDEX IF NOT EXISTS idx_post_reactions_post_id ON public.post_reactions USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_post_reactions_post_user ON public.post_reactions USING btree (post_id, user_id);
CREATE INDEX IF NOT EXISTS idx_post_shares_post_id ON public.post_shares USING btree (post_id);
CREATE INDEX IF NOT EXISTS idx_post_shares_user_id ON public.post_shares USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_post_tags_post_id ON public.post_tags USING btree (post_id, status);
CREATE INDEX IF NOT EXISTS idx_post_tags_tagged_user_status ON public.post_tags USING btree (tagged_user_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_post_tags_tagger ON public.post_tags USING btree (tagger_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_posts_categories ON public.posts USING gin (categories);
CREATE INDEX IF NOT EXISTS idx_posts_content_hash ON public.posts USING btree (content_hash) WHERE (content_hash IS NOT NULL);
CREATE INDEX IF NOT EXISTS idx_posts_content_trgm ON public.posts USING gin (content gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_posts_created_at_desc ON public.posts USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_posts_indexing_disabled ON public.posts USING btree (indexing_disabled) WHERE (indexing_disabled = true);
CREATE INDEX IF NOT EXISTS idx_posts_is_public_created ON public.posts USING btree (created_at DESC) WHERE is_public;
CREATE INDEX IF NOT EXISTS idx_posts_privacy_created_at ON public.posts USING btree (privacy, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_posts_rand_public ON public.posts USING btree (rand_key) WHERE is_public;
CREATE INDEX IF NOT EXISTS idx_posts_user_id ON public.posts USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_posts_user_id_created_at ON public.posts USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_profiles_created_at_id_desc ON public.profiles USING btree (created_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_profiles_full_name_trgm ON public.profiles USING gin (full_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_profiles_indexing_disabled ON public.profiles USING btree (indexing_disabled) WHERE (indexing_disabled = true);
CREATE INDEX IF NOT EXISTS idx_profiles_public_full_name_trgm ON public.profiles_public_data USING gin (full_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_profiles_public_fullname ON public.profiles_public_data USING btree (full_name);
CREATE INDEX IF NOT EXISTS idx_profiles_reengagement_scan ON public.profiles USING btree (last_active_at, reengagement_sends_count, last_reengagement_sent_at) WHERE (reengagement_sends_count < 4);
CREATE INDEX IF NOT EXISTS idx_push_delivery_log_created ON public.push_delivery_log USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_push_delivery_log_outcome ON public.push_delivery_log USING btree (outcome, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_push_tokens_user ON public.push_tokens USING btree (user_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_auto_album ON public.photo_albums USING btree (user_id, album_type) WHERE (album_type = ANY (ARRAY['profile_pictures'::text, 'cover_photos'::text]));
CREATE UNIQUE INDEX IF NOT EXISTS notification_emit_log_idem ON public.notification_emit_log USING btree (kind, entity_id, COALESCE(round_number, '-1'::integer), COALESCE(recipient_user_id, '00000000-0000-0000-0000-000000000000'::uuid));
CREATE INDEX IF NOT EXISTS notification_emit_log_recipient_idx ON public.notification_emit_log USING btree (recipient_user_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS office_staff_id_number_uq ON public.office_staff USING btree (upper(TRIM(BOTH FROM id_number)));
CREATE INDEX IF NOT EXISTS post_media_media_id ON public.post_media USING btree (media_id);
CREATE UNIQUE INDEX IF NOT EXISTS post_media_post_media_uniq ON public.post_media USING btree (post_id, media_id);
CREATE UNIQUE INDEX IF NOT EXISTS posts_user_idempotency_key ON public.posts USING btree (user_id, idempotency_key) WHERE (idempotency_key IS NOT NULL);
CREATE UNIQUE INDEX IF NOT EXISTS profiles_custom_url_unique ON public.profiles USING btree (lower(custom_url)) WHERE (custom_url IS NOT NULL);
CREATE UNIQUE INDEX IF NOT EXISTS uniq_post_tags_active ON public.post_tags USING btree (post_id, tagger_id, tagged_user_id) WHERE (status = ANY (ARRAY['pending'::post_tag_status, 'approved'::post_tag_status]));

CREATE INDEX IF NOT EXISTS idx_raw_commitments_competition ON public.raw_commitments USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_raw_commitments_entry_photo ON public.raw_commitments USING btree (entry_id, photo_index);
CREATE INDEX IF NOT EXISTS idx_raw_commitments_user ON public.raw_commitments USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_referral_codes_code ON public.referral_codes USING btree (code);
CREATE INDEX IF NOT EXISTS idx_referrals_referral_code_id ON public.referrals USING btree (referral_code_id);
...
CREATE INDEX IF NOT EXISTS idx_role_applications_reviewed_by ON public.role_applications USING btree (reviewed_by);
CREATE INDEX IF NOT EXISTS idx_role_applications_user_id ON public.role_applications USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_round_snapshots_competition_id ON public.round_snapshots USING btree (competition_id);
CREATE INDEX IF NOT EXISTS idx_scheduled_posts_pending_due ON public.scheduled_posts USING btree (scheduled_for) WHERE (status = 'pending'::text);
CREATE INDEX IF NOT EXISTS idx_scheduled_posts_user_status_time ON public.scheduled_posts USING btree (user_id, status, scheduled_for DESC);
CREATE INDEX IF NOT EXISTS idx_search_recents_user_time ON public.search_recents USING btree (user_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_story_views_story ON public.story_views USING btree (story_id);
CREATE INDEX IF NOT EXISTS idx_story_views_viewer ON public.story_views USING btree (viewer_id);
CREATE INDEX IF NOT EXISTS idx_suppressed_emails_email ON public.suppressed_emails USING btree (email);
CREATE INDEX IF NOT EXISTS idx_test_agent_config_updated_by ON public.test_agent_config USING btree (updated_by);
CREATE INDEX IF NOT EXISTS idx_test_agent_runs_created_at ON public.test_agent_runs USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_test_agent_runs_status ON public.test_agent_runs USING btree (status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ticket_replies_ticket_id ON public.ticket_replies USING btree (ticket_id);
CREATE INDEX IF NOT EXISTS idx_user_notif_user_created ON public.user_notifications USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_notif_user_read ON public.user_notifications USING btree (user_id, is_read) WHERE (is_read = false);
CREATE INDEX IF NOT EXISTS idx_v3_tag_label_alias_canonical_stage_key ON public.v3_tag_label_alias USING btree (canonical_stage_key);
CREATE INDEX IF NOT EXISTS idx_wallet_recon_finding_type ON public.wallet_reconciliation_log USING btree (finding_type);
CREATE INDEX IF NOT EXISTS idx_wallet_recon_transaction_id ON public.wallet_reconciliation_log USING btree (transaction_id);
CREATE INDEX IF NOT EXISTS idx_wallet_recon_user_id ON public.wallet_reconciliation_log USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_wallet_txn_status ON public.wallet_transactions USING btree (status) WHERE (status = 'pending'::text);
CREATE INDEX IF NOT EXISTS idx_wallet_txn_user_created ON public.wallet_transactions USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_withdrawal_status ON public.withdrawal_requests USING btree (status) WHERE (status = 'pending'::text);
CREATE INDEX IF NOT EXISTS idx_wlal_captured_at ON public.wallet_ledger_audit_log USING btree (captured_at DESC);
CREATE INDEX IF NOT EXISTS idx_wlal_op_result ON public.wallet_ledger_audit_log USING btree (op, result);
CREATE INDEX IF NOT EXISTS idx_wlal_target_user ON public.wallet_ledger_audit_log USING btree (target_user_id);
CREATE INDEX IF NOT EXISTS idx_wlsl_captured_at ON public.wallet_ledger_shadow_log USING btree (captured_at DESC);
CREATE INDEX IF NOT EXISTS idx_wlsl_op_key ON public.wallet_ledger_shadow_log USING btree (op, idempotency_key);
CREATE INDEX IF NOT EXISTS idx_wt_status_voided ON public.wallet_transactions USING btree (status) WHERE (status = 'voided'::text);
CREATE UNIQUE INDEX IF NOT EXISTS uniq_user_notifications_dedup_key ON public.user_notifications USING btree (dedup_key) WHERE (dedup_key IS NOT NULL);
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_txn_gift_expiry ON public.wallet_transactions USING btree (reference_id) WHERE (reference_type = 'gift_expiry'::text);
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_txn_paypal_capture_id ON public.wallet_transactions USING btree (((metadata ->> 'paypal_capture_id'::text))) WHERE ((type = 'deposit'::text) AND ((metadata ->> 'gateway'::text) = 'paypal'::text) AND ((metadata ->> 'paypal_capture_id'::text) IS NOT NULL));
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_txn_paypal_order_id ON public.wallet_transactions USING btree (((metadata ->> 'paypal_order_id'::text))) WHERE ((type = 'deposit'::text) AND ((metadata ->> 'gateway'::text) = 'paypal'::text) AND (metadata ? 'paypal_order_id'::text));
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_txn_razorpay_payment_id ON public.wallet_transactions USING btree (((metadata ->> 'razorpay_payment_id'::text))) WHERE ((type = 'deposit'::text) AND ((metadata ->> 'gateway'::text) = 'razorpay'::text) AND (metadata ? 'razorpay_payment_id'::text));
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_txn_withdrawal_refund ON public.wallet_transactions USING btree (reference_id) WHERE (reference_type = 'withdrawal_refund'::text);
CREATE INDEX IF NOT EXISTS v3_mirror_log_entry_judge_idx ON public.v3_mirror_log USING btree (entry_id, judge_id, round_number);
CREATE INDEX IF NOT EXISTS v3_mirror_log_occurred_idx ON public.v3_mirror_log USING btree (occurred_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS v3_stage_catalog_active_round_token_uniq ON public.v3_stage_catalog USING btree (round_number, decision_token) WHERE (is_active = true);
CREATE INDEX IF NOT EXISTS v3_stage_catalog_label_lower_idx ON public.v3_stage_catalog USING btree (lower(tag_label_canonical)) WHERE (is_active = true);
CREATE INDEX IF NOT EXISTS v3_stage_catalog_round_active_idx ON public.v3_stage_catalog USING btree (round_number) WHERE (is_active = true);
CREATE INDEX IF NOT EXISTS v3_tag_label_alias_lookup_idx ON public.v3_tag_label_alias USING btree (lower(TRIM(BOTH FROM alias_label)));
CREATE UNIQUE INDEX IF NOT EXISTS v3_tag_label_alias_unique ON public.v3_tag_label_alias USING btree (round_number, lower(TRIM(BOTH FROM alias_label)));
CREATE INDEX IF NOT EXISTS wallet_ledger_v2_diff_log_ran_at_idx ON public.wallet_ledger_v2_diff_log USING btree (ran_at DESC);
CREATE INDEX IF NOT EXISTS wallet_ledger_v2_rows_op_created_at_idx ON public.wallet_ledger_v2_rows USING btree (op, created_at DESC);
CREATE INDEX IF NOT EXISTS wallet_ledger_v2_rows_user_id_created_at_idx ON public.wallet_ledger_v2_rows USING btree (user_id, created_at DESC);
...
ALTER TABLE public.competition_entry_counts ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.held_result_notifications ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.posts_dead_host_backup_20260812 ENABLE ROW LEVEL SECURITY;
...
CREATE OR REPLACE FUNCTION public._field_visible(_scope text, _is_friend_tier boolean, _is_owner_tier boolean)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
AS $function$
  SELECT CASE COALESCE(NULLIF(_scope,''), 'public')
    WHEN 'public'  THEN true
    WHEN 'friends' THEN _is_friend_tier
    WHEN 'only_me' THEN _is_owner_tier
    ELSE true
  END;
$function$
;

CREATE OR REPLACE FUNCTION public._flag_review_keyword_hit(_content text)
 RETURNS text
 LANGUAGE plpgsql
...
BEGIN
  ...
  END IF;
  FOR _kw IN
    SELECT keyword FROM public.blocked_keywords
    WHERE is_active = true AND severity = 'flag_review'
      AND keyword IS NOT NULL AND length(btrim(keyword)) > 0
  LOOP
    _norm := lower(regexp_replace(_kw, '[^a-zA-Z0-9]', '', 'g'));
    CONTINUE WHEN _norm = '';
    _gap := '\y' || regexp_replace(_norm, '(.)', '\1[^a-zA-Z0-9]*', 'g') || '\y';
    IF _content ~* _gap THEN
      RETURN _kw;
    END IF;
  END LOOP;
  RETURN NULL;
...
CREATE OR REPLACE FUNCTION public._notification_template_for_entry(_status text, _placement text)
...
CREATE OR REPLACE FUNCTION public._resolve_stage_key_from_entry(_status text, _current_round text, _progression_decision text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.admin_list_certificates(_query text DEFAULT ''::text, _type text DEFAULT NULL::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, user_id uuid, title text, description text, type text, heading text, issued_at timestamp with time zone, is_revoked boolean, revoked_at timestamp with time zone, revoked_reason text, certificate_id text, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.admin_reject_wallet_transaction(_admin_id uuid, _txn_id uuid, _reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_actor uuid;
BEGIN
  -- BUG-001: derive admin identity from the real session (ignore caller-supplied _admin_id)
  v_actor := auth.uid();
  ...
  IF v_current_status = 'rejected' THEN
    RETURN jsonb_build_object('success', true, 'already', true, 'txn_id', _txn_id, 'status', 'rejected');
  END IF;
  ...
  VALUES (
    ...
    jsonb_build_object('status', 'rejected', 'prev_status', v_current_status, 'reason', _reason, 'user_id', v_user_id, 'amount', v_amount, 'type', v_type),
    v_actor
  );

  RETURN jsonb_build_object('success', true, 'already', false, 'txn_id', _txn_id, 'status', 'rejected', 'prev_status', v_current_status);
END;
$function$
;

CREATE OR REPLACE FUNCTION public.admin_rewind_stage(_entry_id uuid, _to_stage_key text, _reason text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.admin_search_certificate_recipients(_query text, _limit integer DEFAULT 20)
 RETURNS TABLE(id uuid, full_name text, email text, avatar_url text, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'auth'
AS $function$
...
CREATE OR REPLACE FUNCTION public.admin_search_users(search_query text DEFAULT ''::text, search_by text DEFAULT 'name'::text)
 RETURNS TABLE(id uuid, email text, full_name text, avatar_url text, bio text, is_suspended boolean, suspended_until timestamp with time zone, suspension_reason text, created_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.admin_search_users_v2(_query text DEFAULT ''::text, _by text DEFAULT 'name'::text, _role text DEFAULT NULL::text, _badge text DEFAULT NULL::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, email text, full_name text, avatar_url text, bio text, is_suspended boolean, suspended_until timestamp with time zone, suspension_reason text, created_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.admin_set_photo_rejected(_entry_id uuid, _photo_index integer, _rejected boolean, _reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.admin_wallet_credit(_admin_id uuid, _target_user_id uuid, _amount numeric, _type text, _description text DEFAULT NULL::text, _reference_id uuid DEFAULT NULL::uuid, _reference_type text DEFAULT NULL::text, _metadata jsonb DEFAULT NULL::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- BUG-001: authority from session. Authenticated callers must actually be admins;
  -- trusted service_role calls (auth.uid() IS NULL) are allowed. _admin_id is ignored.
...
CREATE OR REPLACE FUNCTION public.app_has_role(_user_id uuid, _role text)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ select public.has_role(_user_id, _role); $function$
;
...
CREATE OR REPLACE FUNCTION public.approve_deposit(_admin_id uuid, _txn_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- BUG-001: verify the real logged-in caller is admin (ignore caller-supplied _admin_id)
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
    RAISE EXCEPTION 'Only admins can approve deposits';
  END IF;

  -- BUG-046: lock the row so concurrent approvals serialize (no double-credit).
...
CREATE OR REPLACE FUNCTION public.audit_phase_parity(sample_limit integer DEFAULT 100)
 RETURNS TABLE(id uuid, status text, starts_at timestamp with time zone, ends_at timestamp with time zone, voting_ends_at timestamp with time zone, judging_completed boolean, legacy_phase text, db_phase text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  IF (NEW.status IS DISTINCT FROM OLD.status)
    ...
    INSERT INTO public.certificates (user_id, title, description, type, reference_id, published_at)
    VALUES (
      NEW.user_id,
      ...
      _cert_type,
      NEW.competition_id,
      CASE WHEN _r4_published THEN now() ELSE NULL END   -- hold until declare
    );
  END IF;
...
CREATE OR REPLACE FUNCTION public.auto_follow_official()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _official uuid := public.get_primary_admin_user_id();
BEGIN
  IF _official IS NULL OR NEW.id = _official THEN
    RETURN NEW;  -- no official set, or the official account itself
  END IF;
  INSERT INTO public.follows (follower_id, following_id)
  VALUES (NEW.id, _official)
...
CREATE OR REPLACE FUNCTION public.classify_judging_tag(p_label text, p_visible_in_round integer[])
 RETURNS TABLE(family text, advances_to integer, blocks_from integer, verification_round integer)
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.competition_total_entries(_competition_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  ...
  from public.competition_entry_counts
...
CREATE OR REPLACE FUNCTION public.create_pending_deposit(_user_id uuid, _amount numeric, _gateway text, _reference text, _metadata jsonb DEFAULT '{}'::jsonb, _idempotency_key text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.create_system_post(_content text, _image_url text, _image_urls text[], _thumbnail_urls text[], _media_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.current_phase_for(p_status text, p_starts_at timestamp with time zone, p_ends_at timestamp with time zone, p_voting_ends_at timestamp with time zone, p_judging_completed boolean, p_legacy_phase text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.custom_url_available(_candidate text, _for_user uuid DEFAULT NULL::uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  _schwa   boolean := false;
  _prev_virama boolean := false;
  _in_conjunct boolean := false;
BEGIN
...
CREATE OR REPLACE FUNCTION public.derive_decision_from_score(_avg numeric, _round_number integer)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'public'
...
BEGIN
  WITH celebrants AS (
    SELECT p.id, public.notif_display_name(p.id) AS display_name
    FROM public.profiles p
...
CREATE OR REPLACE FUNCTION public.emit_notification(_kind text, _entity_id uuid, _round_number integer, _recipient_user_id uuid, _in_app_type text, _in_app_title text, _in_app_message text, _in_app_reference_id uuid, _email_template text, _email_data jsonb DEFAULT '{}'::jsonb, _action_url text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  _notif_id uuid;
  _msg_id text;
  _site_url text := 'https://50mmretina.com';
...
BEGIN
  ...
  -- 2) Enqueue email — but ONLY if the recipient hasn't opted out of this type
  --    and isn't on the suppression list. Attach an unsubscribe token.
  IF _user_email IS NOT NULL AND _email_template IS NOT NULL
     AND public.get_notification_email_enabled(_recipient_user_id, _in_app_type)
     AND NOT EXISTS (SELECT 1 FROM public.suppressed_emails WHERE email = lower(_user_email)) THEN

    -- unsubscribe token (reuse an unused one, else mint)
    SELECT token INTO _unsub FROM public.email_unsubscribe_tokens WHERE email = lower(_user_email) AND used_at IS NULL LIMIT 1;
    IF _unsub IS NULL THEN
      _unsub := replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', '');
      INSERT INTO public.email_unsubscribe_tokens (token, email) VALUES (_unsub, lower(_user_email)) ON CONFLICT (email) DO NOTHING;
      SELECT token INTO _unsub FROM public.email_unsubscribe_tokens WHERE email = lower(_user_email) AND used_at IS NULL LIMIT 1;
    END IF;
    ...
    PERFORM public.enqueue_email('transactional_emails', jsonb_build_object(
      'message_id', _msg_id,
      'to', _user_email,
      'from', '50mm Retina World <no-reply@50mmretina.com>',
      'sender_domain', '50mmretina.com',
      ...
      'template_data', _email_data || jsonb_build_object(
        'userName', _user_name,
        'participantName', _user_name,
        'actionUrl', _final_action_url,
        'entryUrl', _final_action_url,
        'unsubscribeUrl', _site_url || '/unsubscribe?token=' || _unsub
      ),
      'queued_at', now()::text
    ));
  END IF;
...
BEGIN
  ...
  FOR _kw IN
    SELECT keyword FROM public.blocked_keywords
    WHERE is_active = true AND severity = 'auto_hide' AND keyword IS NOT NULL AND length(btrim(keyword)) > 0
  LOOP
    -- normalize the keyword to alphanumerics only (chars are then all regex-safe)
    _norm_kw := lower(regexp_replace(_kw, '[^a-zA-Z0-9]', '', 'g'));
    CONTINUE WHEN _norm_kw = '';
    -- build boundary-anchored gap pattern: k[^alnum]*w[^alnum]*...
    _gap := '\y' || regexp_replace(_norm_kw, '(.)', '\1[^a-zA-Z0-9]*', 'g') || '\y';
    IF NEW.content ~* _gap THEN
      RAISE EXCEPTION 'Comment blocked by content filter' USING ERRCODE = 'check_violation';
    END IF;
  END LOOP;
  RETURN NEW;
END;
...
CREATE OR REPLACE FUNCTION public.engagement_probe_report(_days integer DEFAULT 14)
 RETURNS TABLE(probe_source text, probes bigint, covered bigint, coverage_pct numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.enqueue_post_created_job()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  PERFORM public.enqueue_post_job(jsonb_build_object(
    'type',       'post_created',
    'post_id',    NEW.id,
...
CREATE OR REPLACE FUNCTION public.enqueue_post_job(_payload jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  PERFORM pgmq.send('post_jobs', _payload);
EXCEPTION WHEN OTHERS THEN
  RAISE WARNING 'enqueue_post_job failed (payload=%): %', _payload, SQLERRM;
END;
$function$
;

CREATE OR REPLACE FUNCTION public.enroll_in_course(_user_id uuid, _course_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- BUG-074 guard: a caller may only enroll themselves (admins/service exempt).
  IF _user_id <> auth.uid() AND NOT has_role(auth.uid(), 'admin'::app_role) THEN
    RAISE EXCEPTION 'Not authorized: you can only enroll yourself';
  END IF;
...
CREATE OR REPLACE FUNCTION public.expire_gift_credit(_gift_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  _deduct  numeric;
BEGIN
  -- Lock the gift row: a concurrent run blocks, then sees is_expired and skips.
  SELECT id, user_id, amount, reason, is_expired
  INTO _gift
  FROM public.gift_announcements
  WHERE id = _gift_id
  FOR UPDATE;
  ...
  END IF;
  IF _gift.is_expired THEN
    RETURN jsonb_build_object('skipped', true, 'reason', 'already_expired');
  END IF;

  SELECT balance INTO _balance FROM public.wallets WHERE user_id = _gift.user_id;
  _deduct := LEAST(COALESCE(_gift.amount, 0), COALESCE(_balance, 0));

  IF _deduct > 0 THEN
    -- reference_id/type tag makes the debit idempotent via uq_wallet_txn_gift_expiry.
    PERFORM public.wallet_transaction(
      _gift.user_id,
      'gift_expiry',
      -_deduct,
      'Gift credit expired: "' || COALESCE(_gift.reason, '') || '"',
      _gift.id,
      'gift_expiry',
      jsonb_build_object('expired_gift_id', _gift.id, 'original_amount', _gift.amount)
    );
  END IF;

  UPDATE public.gift_announcements SET is_expired = true WHERE id = _gift_id;

  RETURN jsonb_build_object('expired', true, 'deducted', _deduct);
END;
$function$
;
...
CREATE OR REPLACE FUNCTION public.feed_candidates(_seed text, _categories text[] DEFAULT NULL::text[], _recent integer DEFAULT 500, _sample integer DEFAULT 1000, _own integer DEFAULT 100, _friends integer DEFAULT 200)
 RETURNS TABLE(id uuid, user_id uuid, created_at timestamp with time zone, bucket integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.feed_rank_score(_id uuid, _bucket integer, _seed text)
...
CREATE OR REPLACE FUNCTION public.filter_moderated_user_ids(_ids uuid[])
...
AS $function$
  ...
  WHERE p.id = ANY(_ids)
    AND (p.is_banned OR p.is_suspended);
$function$
;
...
CREATE OR REPLACE FUNCTION public.flag_image_comment_for_review()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  _hit := public._flag_review_keyword_hit(NEW.content);
  IF _hit IS NOT NULL AND NOT COALESCE(NEW.is_flagged, false) THEN
    UPDATE public.image_comments
       SET is_flagged = true,
           flag_reason = COALESCE(flag_reason, 'Auto-flagged: matched review keyword')
     WHERE id = NEW.id;
  END IF;
...
CREATE OR REPLACE FUNCTION public.flag_post_comment_for_review()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF _hit IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.comment_reports
    WHERE post_comment_id = NEW.id AND source = 'keyword_flag'
  ) THEN
    INSERT INTO public.comment_reports (post_comment_id, reporter_id, reason, details, status, source)
    VALUES (NEW.id, NULL, 'auto_flagged_keyword', 'Auto-flagged by blocklist review keyword', 'pending', 'keyword_flag');
  END IF;
  RETURN NULL;
END;
...
BEGIN
  ...
  IF _hit IS NOT NULL AND NOT EXISTS (
    ...
    WHERE post_id = NEW.id AND reason = 'auto_flagged_keyword' AND reporter_id IS NULL
  ) THEN
    INSERT INTO public.post_reports (post_id, reporter_id, reason, details, status)
    VALUES (NEW.id, NULL, 'auto_flagged_keyword', 'Auto-flagged by blocklist review keyword', 'pending');
  END IF;
  RETURN NULL;
END;
$function$
...
CREATE OR REPLACE FUNCTION public.get_ad_analytics(_since timestamp with time zone)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_ad_autoscale_stats(_since timestamp with time zone)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  WITH imp_agg AS (
    SELECT slot_id, ad_source, event_type, COUNT(*) AS cnt
    FROM public.ad_impressions
    WHERE created_at >= _since AND event_type IN ('impression', 'click')
    GROUP BY slot_id, ad_source, event_type
  ),
  conv_agg AS (
    -- BUG-073: group conversions by their OWN recorded ad_source (ad_id == slot_id).
    -- Legacy rows have ad_source NULL and are left for the edge's last-click fallback.
    SELECT ad_id, ad_source, COUNT(*) AS cnt
    FROM public.ad_conversions
    WHERE created_at >= _since
    GROUP BY ad_id, ad_source
  ),
  click_sources AS (
    SELECT DISTINCT ON (slot_id) slot_id, ad_source
    FROM public.ad_impressions
    WHERE created_at >= _since AND event_type = 'click'
    ORDER BY slot_id, created_at DESC
  )
  SELECT jsonb_build_object(
    'impressions', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'slot_id', slot_id, 'ad_source', ad_source, 'event_type', event_type, 'count', cnt
    )) FROM imp_agg), '[]'::jsonb),
    'conversions', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'ad_id', ad_id, 'ad_source', ad_source, 'count', cnt
    )) FROM conv_agg), '[]'::jsonb),
    'click_sources', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'slot_id', slot_id, 'ad_source', ad_source
...
CREATE OR REPLACE FUNCTION public.get_ad_engagement(_creative_ids uuid[])
 RETURNS TABLE(creative_id uuid, like_count integer, comment_count integer, share_count integer, my_reaction text, my_shared boolean, reaction_counts jsonb)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_app_event_counts_admin(_hours integer DEFAULT 24)
 RETURNS TABLE(code text, event text, severity text, occurrences bigint, members bigint, last_seen timestamp with time zone, sample text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_app_events_admin(_hours integer DEFAULT 24, _code text DEFAULT NULL::text, _severity text DEFAULT NULL::text, _limit integer DEFAULT 200)
 RETURNS TABLE(id uuid, created_at timestamp with time zone, code text, event text, severity text, message text, fn text, src_file text, reason text, expected text, actual text, next_step text, duration_ms integer, correlation_id text, user_id uuid, platform text, app_build text, url text, detail jsonb)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_broadcast_feed(_exclude_ids uuid[], _limit integer)
 RETURNS TABLE(id uuid, user_id uuid, content text, image_url text, image_urls text[], privacy text, created_at timestamp with time zone, likes_count integer, comments_count integer, shares_count integer, feed_tier text, author_name text, author_avatar text, thumbnail_urls text[], categories text[])
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_broadcast_feed(_exclude_ids uuid[], _limit integer, _newest_first integer)
 RETURNS TABLE(id uuid, user_id uuid, content text, image_url text, image_urls text[], privacy text, created_at timestamp with time zone, likes_count integer, comments_count integer, shares_count integer, feed_tier text, author_name text, author_avatar text, thumbnail_urls text[], categories text[])
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_broadcast_feed(_exclude_ids uuid[], _limit integer, _newest_first integer, _categories text[])
 RETURNS TABLE(id uuid, user_id uuid, content text, image_url text, image_urls text[], privacy text, created_at timestamp with time zone, likes_count integer, comments_count integer, shares_count integer, feed_tier text, author_name text, author_avatar text, thumbnail_urls text[], categories text[])
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  WITH seed AS (
    -- Per viewer, per hour. Old callers send no seed, so one is derived: each
    -- member gets their own shuffle, stable for an hour, reshuffled after —
    -- closer to the old per-call random() re-deal than a fixed seed would be,
    -- and it keeps _exclude_ids paging coherent within the hour.
...
CREATE OR REPLACE FUNCTION public.get_certificate_drift_admin(p_competition_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(certificate_id uuid, cert_title text, cert_type text, cert_user_id uuid, reference_id uuid, entry_id uuid, entry_user_id uuid, entry_status text, entry_placement text, entry_certificate_ready boolean, competition_id uuid, issued_at timestamp with time zone, drift_type text, severity text, reason text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_certificate_readiness_drift_admin(_competition_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(entry_id uuid, competition_id uuid, competition_title text, competition_phase text, status text, certificate_ready boolean, reason text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  ...
  SELECT 
    ...
    CASE
      WHEN ce.status IN ('winner','runner_up_1','runner_up_2','special_jury','honorary_mention')
  ...
  WHERE ce.certificate_ready = false
    ...
    AND (
      ce.status IN ('winner','runner_up_1','runner_up_2','special_jury','honorary_mention')
...
CREATE OR REPLACE FUNCTION public.get_client_error_stats_admin(_hours integer DEFAULT 24)
 RETURNS TABLE(hour_bucket timestamp with time zone, kind text, platform text, app_build text, occurrences bigint, members bigint, sample text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  ...
  END IF;
  RETURN QUERY
  SELECT date_trunc('hour', e.created_at), e.kind, e.platform, e.app_build,
...
CREATE OR REPLACE FUNCTION public.get_competition_duplicate_clusters(_competition_id uuid)
 RETURNS TABLE(cluster_key text, match_type text, entry_id uuid, photo_index integer, user_id uuid, entry_title text, photo_url text, thumbnail_url text, created_at timestamp with time zone, matched_against_entry uuid, matched_against_photo integer, hamming_distance integer)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_competition_raw_commitments(_competition_id uuid)
 RETURNS TABLE(entry_id uuid, photo_index integer, user_id uuid, entry_title text, photo_url text, thumbnail_url text, photo_title text, raw_required boolean, exif_available boolean, committed_at timestamp with time zone, source text, raw_delivered_at timestamp with time zone, raw_file_url text, admin_verified_at timestamp with time zone, admin_verified_by uuid)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_course_lessons_for_editor(_course_id uuid)
  ...
  IF NOT EXISTS (
    ...
    WHERE c.id = _course_id
      AND (c.author_id = v_uid
           ...
           -- live app_role enum has no 'super_admin'; 'admin' is the elevated role
  ) THEN
    RAISE EXCEPTION 'Not authorized to edit this course' USING ERRCODE = '42501';
  END IF;
...
CREATE OR REPLACE FUNCTION public.get_derived_status_drift_admin()
 RETURNS TABLE(entry_id uuid, competition_id uuid, cached_value text, canonical_value text, status text, current_round text, stage_key text, placement text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_entry_vote_counts(_entry_ids uuid[])
 RETURNS TABLE(entry_id uuid, real_votes integer, adjustment_votes integer, final_votes integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_feed_candidates(_network_ids uuid[], _recent_limit integer DEFAULT 120, _network_limit integer DEFAULT 150, _popular_limit integer DEFAULT 100)
 RETURNS TABLE(id uuid, user_id uuid, content text, image_url text, image_urls text[], privacy text, created_at timestamp with time zone, likes_count integer, comments_count integer, shares_count integer, source_type text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  ...
  WHERE NOT EXISTS (
    ...
    WHERE pr.id = c.user_id
      AND (pr.is_banned IS TRUE OR pr.is_suspended IS TRUE)
  )
...
CREATE OR REPLACE FUNCTION public.get_feed_stories_bar()
 RETURNS TABLE(user_id uuid, full_name text, avatar_url text, is_official boolean, latest_story_at timestamp with time zone, story_count integer, has_unseen boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_gated_status_runtime_drift_admin(p_entry_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS TABLE(entry_id uuid, field text, cache_value text, view_value text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_gift_drift_admin()
 RETURNS TABLE(drift_type text, announcement_id uuid, user_id uuid, gift_credit_id uuid, expected_amount numeric, actual_amount numeric, is_expired boolean, created_at timestamp with time zone, notes text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_judge_collusion_admin(p_competition_id uuid DEFAULT NULL::uuid, p_min_overlap integer DEFAULT 10, p_min_correlation numeric DEFAULT 0.9)
 RETURNS TABLE(competition_id uuid, judge_a uuid, judge_b uuid, shared_entries integer, pearson_r numeric, mean_diff numeric, severity text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_judge_entries_page_v1(_competition_id uuid, _round_number integer, _cursor_created_at timestamp with time zone DEFAULT NULL::timestamp with time zone, _cursor_id uuid DEFAULT NULL::uuid, _limit integer DEFAULT 10)
 RETURNS TABLE(id uuid, title text, description text, photos text[], photo_thumbnails text[], user_id uuid, status text, created_at timestamp with time zone, competition_id uuid, placement text, is_ai_generated boolean, ai_detection_result jsonb, exif_data jsonb, view_count integer, current_round text, next_cursor_created_at timestamp with time zone, next_cursor_id uuid, has_more boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_judging_drift_admin()
 RETURNS TABLE(finding_code text, source_table text, source_row_id uuid, entry_id uuid, judge_id uuid, round_number integer, photo_index integer, detail_label text, expected_value text, actual_value text, occurred_at timestamp with time zone)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_judging_live_tag_progression_invariant_admin()
 RETURNS TABLE(check_name text, status text, proof text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_judging_tag_assignment_counts()
 RETURNS TABLE(tag_id uuid, assignment_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_lesson_content(_lesson_id uuid)
 RETURNS TABLE(id uuid, title text, content text, video_url text, image_url text, sort_order integer, course_id uuid)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  SELECT * INTO v_lesson FROM public.lessons l WHERE l.id = _lesson_id;
  IF NOT FOUND THEN
    RETURN;                       -- unknown lesson -> zero rows ("not found")
  END IF;

  SELECT * INTO v_course FROM public.courses c WHERE c.id = v_lesson.course_id;
  IF NOT FOUND THEN
    RETURN;
  END IF;
  ...
  IF v_uid IS NOT NULL THEN
    v_is_privileged :=
         v_course.author_id = v_uid
      OR public.has_role(v_uid, 'admin'::app_role);
      -- NOTE: live app_role enum has no 'super_admin' value; 'admin' is the
      -- only elevated role (matches the certificates admin RLS policy).
  END IF;

  -- Unpublished courses: only author/admin may read (same as courses RLS)
  IF v_course.status <> 'published' AND NOT v_is_privileged THEN
    RETURN;                       -- behave as "not found", don't leak existence
  END IF;
  ...
  IF v_course.is_free
     ...
     OR (v_uid IS NOT NULL AND EXISTS (
           ...
           WHERE e.user_id = v_uid AND e.course_id = v_course.id))
  THEN
    RETURN QUERY SELECT
      v_lesson.id, v_lesson.title, v_lesson.content, v_lesson.video_url,
      v_lesson.image_url, v_lesson.sort_order, v_lesson.course_id;
  ELSE
    RAISE EXCEPTION 'Enrollment required to view this lesson'
...
CREATE OR REPLACE FUNCTION public.get_my_certificate_entries()
 RETURNS TABLE(id uuid, title text, competition_id uuid, status text, placement text, current_round text, current_round_int integer, progression_decision text, competition_title text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT ce.id, ce.title, ce.competition_id, ce.status, ce.placement,
         ce.current_round, ce.current_round_int, ce.progression_decision,
         c.title AS competition_title
  FROM public.competition_entries ce
  JOIN public.competitions c ON c.id = ce.competition_id
  WHERE ce.user_id = (SELECT auth.uid())
    AND ce.certificate_ready = true
    AND EXISTS (
      SELECT 1 FROM public.competition_round_publish crp
...
CREATE OR REPLACE FUNCTION public.get_my_notifications_grouped(_limit integer DEFAULT 30, _before timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS TABLE(group_key text, type text, notification_ids uuid[], actor_ids uuid[], actor_names text[], actor_usernames text[], actor_avatars text[], actor_count integer, event_count integer, unread_count integer, reference_id uuid, thumbnail_url text, title text, message text, latest_at timestamp with time zone)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
SELECT g.gkey AS group_key, g.type, g.notification_ids,
       ...
       g.actor_count, g.event_count, g.unread_count, l.reference_id,
...
CREATE OR REPLACE FUNCTION public.get_my_unread_notifications_grouped(_limit integer DEFAULT 20, _exclude_types text[] DEFAULT '{}'::text[])
 RETURNS TABLE(group_key text, type text, notification_ids uuid[], actor_ids uuid[], actor_names text[], actor_usernames text[], actor_avatars text[], actor_known boolean[], actor_count integer, event_count integer, unread_count integer, reference_id uuid, thumbnail_url text, title text, message text, latest_at timestamp with time zone, total_unread integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_needs_review_recipients_for_round(p_competition_id uuid, p_round_number integer)
...
CREATE OR REPLACE FUNCTION public.get_notification_drift_admin(_window_days integer DEFAULT 90)
 RETURNS TABLE(expected_template text, total_entries bigint, missing_emit bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_notification_health_stats_admin()
 RETURNS TABLE(emits_today bigint, emits_total bigint, distinct_templates bigint, failures_today bigint, dlq_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_photo_r4_awards(p_entry_ids uuid[])
 RETURNS TABLE(entry_id uuid, photo_index integer, stage_key text, all_stage_keys text[])
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN ar.e_status IN ('winner','runner_up_1','runner_up_2','special_jury','honorary_mention')
  ...
  FROM award_rank ar
  WHERE
    (ar.e_status IN ('winner','runner_up_1','runner_up_2','special_jury','honorary_mention')
     AND ar.e_placement IS DISTINCT FROM ar.e_status)
...
CREATE OR REPLACE FUNCTION public.get_profile_visible_fields(_target uuid)
 RETURNS TABLE(avatar_url text, bio text, photography_interests text[], portfolio_url text, facebook_url text, instagram_url text, twitter_url text, youtube_url text, website_url text, current_city text, workplace text, education text, pronouns text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _caller uuid := auth.uid();
  _is_owner boolean := (_caller IS NOT NULL AND _caller = _target);
  _is_admin boolean := (_caller IS NOT NULL AND public.has_role(_caller, 'admin'::app_role));
  _is_friend boolean := (_caller IS NOT NULL AND public.are_friends(_caller, _target));
  ps jsonb;
  p public.profiles%ROWTYPE;
  -- visibility helper values
  _priv boolean := (_is_owner OR _is_admin);           -- sees only_me
  _frnd boolean := (_is_owner OR _is_admin OR _is_friend); -- sees friends
BEGIN
  SELECT * INTO p FROM public.profiles WHERE id = _target;
  IF NOT FOUND THEN
    RETURN;
  END IF;
  ps := COALESCE(p.privacy_settings, '{}'::jsonb);

  RETURN QUERY SELECT
    CASE WHEN public._field_visible(ps->>'avatar',      _frnd, _priv) THEN p.avatar_url    ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'bio',         _frnd, _priv) THEN p.bio           ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'interests',   _frnd, _priv) THEN p.photography_interests ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'portfolio',   _frnd, _priv) THEN p.portfolio_url ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'social_links',_frnd, _priv) THEN p.facebook_url  ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'social_links',_frnd, _priv) THEN p.instagram_url ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'social_links',_frnd, _priv) THEN p.twitter_url   ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'social_links',_frnd, _priv) THEN p.youtube_url   ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'social_links',_frnd, _priv) THEN p.website_url   ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'city_country',_frnd, _priv) THEN p.current_city  ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'workplace',   _frnd, _priv) THEN p.workplace     ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'education',    _frnd, _priv) THEN p.education     ELSE NULL END,
    CASE WHEN public._field_visible(ps->>'pronouns',    _frnd, _priv) THEN p.pronouns      ELSE NULL END;
END;
$function$
;

CREATE OR REPLACE FUNCTION public.get_progression_drift_admin()
 RETURNS TABLE(entry_id uuid, competition_id uuid, title text, status text, stored_decision text, expected_decision text, total_decisions bigint, has_drift boolean, updated_at timestamp with time zone)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_public_final_votes(_entry_ids uuid[])
 RETURNS TABLE(entry_id uuid, photo_index integer, final_votes integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT efv.entry_id,
         COALESCE(efv.photo_index, 0)::integer,
         COALESCE(efv.final_votes, 0)::integer
  FROM public.entry_final_votes efv
  WHERE efv.entry_id = ANY(_entry_ids)
    AND NOT public.is_vote_phase_locked(efv.entry_id);
$function$
;
...
CREATE OR REPLACE FUNCTION public.get_public_round_scores(p_competition_id uuid, p_round_number integer)
 RETURNS TABLE(entry_id uuid, photo_index integer, anonymized_judge_label text, composition_score integer, color_palette_score integer, technique_score integer, line_score integer, shape_score integer, form_score integer, texture_score integer, space_score integer, tone_score integer, balance_score integer, light_score integer, depth_score integer, editing_score integer, story_score integer, moment_score integer, average_score numeric)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- Gate: scores are public only once the ADMIN DECLARED the round.
  IF NOT EXISTS (
    SELECT 1 FROM public.competition_round_publish crp
    WHERE crp.competition_id = p_competition_id
      AND crp.round_number = p_round_number
      AND crp.published_at IS NOT NULL
  ) THEN
    RETURN;
  END IF;
  ...
  SELECT
    js.entry_id, js.photo_index, jl.label,
    js.composition_score, js.color_palette_score, js.technique_score,
    js.line_score, js.shape_score, js.form_score, js.texture_score,
    js.space_score, js.tone_score, js.balance_score, js.light_score,
    js.depth_score, js.editing_score, js.story_score, js.moment_score,
    ROUND((
      COALESCE(js.composition_score,0)+COALESCE(js.color_palette_score,0)+COALESCE(js.technique_score,0)+
      ...
      COALESCE(js.editing_score,0)+COALESCE(js.story_score,0)+COALESCE(js.moment_score,0)
    )::numeric / NULLIF(
      (js.composition_score IS NOT NULL)::int+(js.color_palette_score IS NOT NULL)::int+(js.technique_score IS NOT NULL)::int+
      (js.line_score IS NOT NULL)::int+(js.shape_score IS NOT NULL)::int+(js.form_score IS NOT NULL)::int+
      (js.texture_score IS NOT NULL)::int+(js.space_score IS NOT NULL)::int+(js.tone_score IS NOT NULL)::int+
      (js.balance_score IS NOT NULL)::int+(js.light_score IS NOT NULL)::int+(js.depth_score IS NOT NULL)::int+
      (js.editing_score IS NOT NULL)::int+(js.story_score IS NOT NULL)::int+(js.moment_score IS NOT NULL)::int, 0), 2) AS average_score
  FROM public.judge_scores js
  JOIN public.competition_entries ce ON ce.id = js.entry_id
  JOIN judge_labels jl ON jl.judge_id = js.judge_id
  WHERE ce.competition_id = p_competition_id
    AND js.round_number = p_round_number
...
CREATE OR REPLACE FUNCTION public.get_push_health()
 RETURNS TABLE(devices_registered integer, members_with_a_device integer, notifications_24h integer, queued_24h integer, skipped_preference_24h integer, skipped_self_24h integer, errors_24h integer, last_queued_at timestamp with time zone, last_error_at timestamp with time zone, last_error_detail text, delivery_looks_dead boolean)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_referral_drift_admin()
 RETURNS TABLE(drift_type text, referral_id uuid, referrer_id uuid, referred_id uuid, expected_amount numeric, actual_amount numeric, rewarded_at timestamp with time zone, notes text)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_round_pending_entries(p_competition_id uuid, p_round integer)
...
CREATE OR REPLACE FUNCTION public.get_system_tag_catalog_drift()
 RETURNS TABLE(side text, round_number integer, label_or_canonical text, tag_id uuid, stage_key text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_top_contributors_v1()
 RETURNS TABLE(user_id uuid, posts_count bigint, likes_received bigint, comments_received bigint, score numeric)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.global_search(term text, section text DEFAULT 'all'::text, username_only boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
...
AS $function$
BEGIN
  -- Only act when a moderation column actually changes
  IF NEW.is_banned IS DISTINCT FROM OLD.is_banned
    ...
    -- Trusted actors may change anything: service_role (auth.uid() IS NULL) or an admin
    IF auth.uid() IS NULL OR has_role(auth.uid(), 'admin'::app_role) THEN
      RETURN NEW;
    END IF;

    -- Non-admin acting on a row (RLS already limits them to their own):
    -- (a) may NEVER change the ban flag
    IF NEW.is_banned IS DISTINCT FROM OLD.is_banned THEN
      RAISE EXCEPTION 'Not permitted to change ban status' USING ERRCODE = '42501';
    END IF;

    -- (b) may only CLEAR an already-expired suspension (matches app auto-expiry)
    IF NEW.is_suspended IS DISTINCT FROM OLD.is_suspended
       OR NEW.suspended_until IS DISTINCT FROM OLD.suspended_until THEN
      IF NOT (OLD.is_suspended = true
              AND OLD.suspended_until IS NOT NULL
              AND OLD.suspended_until < now()
              AND NEW.is_suspended = false) THEN
        RAISE EXCEPTION 'Not permitted to change suspension status' USING ERRCODE = '42501';
      END IF;
    END IF;
  END IF;
...
CREATE OR REPLACE FUNCTION public.handle_new_user()
 ...
 SET search_path TO 'public'
AS $function$
DECLARE _oauth_name text; _is_first_admin boolean;
BEGIN
  _oauth_name := COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.raw_user_meta_data->>'name', NULL);
  ...
  VALUES (NEW.id,
    ...
    '/avatars/fallback/m' || ((abs(hashtext(NEW.id::text)) % 5) + 1)::text || '.svg');
...
CREATE OR REPLACE FUNCTION public.has_profile_photo(_user_id uuid)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT COALESCE((SELECT avatar_url LIKE 'https://cdn.50mmretina.com/%' OR avatar_url LIKE '%/storage/v1/object/public/avatars/%' FROM public.profiles WHERE id = _user_id), false);
$function$
;
...
CREATE OR REPLACE FUNCTION public.html_escape(_s text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
AS $function$
...
CREATE OR REPLACE FUNCTION public.increment_managed_page_view(_page_id text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  SET value = (
        SELECT jsonb_agg(
          CASE WHEN (elem->>'id') = _page_id
            THEN jsonb_set(elem, '{view_count}',
                           to_jsonb(COALESCE((elem->>'view_count')::int, 0) + 1))
            ELSE elem
          END
        )
...
CREATE OR REPLACE FUNCTION public.is_engagement_phase_locked(_image_type text, _image_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.issue_course_completion_certificate(_course_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- Gate 1: must be enrolled (enrollment is created only by enroll_in_course,
  -- which already enforces payment + published status).
  IF NOT EXISTS (
    SELECT 1 FROM public.course_enrollments
  ...
  -- Gate 2: every lesson of the course completed.
  ...
  SELECT count(DISTINCT lp.lesson_id) INTO _completed_lessons
  ...
  IF _completed_lessons < _total_lessons THEN
    RAISE EXCEPTION 'Course not complete: % of % lessons finished',
      _completed_lessons, _total_lessons;
  END IF;

  -- Idempotency: one completion certificate per (user, course), ever.
  -- Deliberately does NOT exclude revoked rows — re-issuing after an admin
  -- revocation must stay an admin decision, not a self-service loophole.
  SELECT id INTO _existing_id
  FROM public.certificates
  WHERE user_id = _uid
    AND type = 'course_completion'
    AND reference_id = _course_id
  ...
  -- Title/description strings byte-match the current client writer
  -- (CourseDetail.tsx:85-86) so Certificates.tsx's
  -- `.replace(" — Completion Certificate", "")` keeps working.
  INSERT INTO public.certificates (user_id, type, title, description, reference_id)
  VALUES (
    _uid,
    'course_completion',
    _course.title || ' — Completion Certificate',
    'Successfully completed all ' || _total_lessons || ' lessons in "' || _course.title || '".',
    _course_id
  )
...
CREATE OR REPLACE FUNCTION public.judge_apply_single_tag(_entry_id uuid, _photo_index integer, _round_number integer, _tag_id uuid, _judge_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- Remove this judge's other round-visible tags on this (entry, photo, round).
  DELETE FROM public.judge_tag_assignments
   WHERE entry_id     = _entry_id
     AND photo_index  = _photo_index
     AND round_number = _round_number
     AND judge_id     = _judge_id
     AND tag_id <> _tag_id
     AND tag_id IN (
       SELECT id FROM public.judging_tags
       WHERE visible_in_round @> ARRAY[_round_number]
     );

  -- Insert the new tag. If this fails, the DELETE above rolls back (atomic).
  INSERT INTO public.judge_tag_assignments
    (entry_id, photo_index, round_number, tag_id, judge_id)
...
CREATE OR REPLACE FUNCTION public.judge_round_open_by_number(_entry_id uuid, _round_number integer)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.judging_write_decision_atomic(p_entry_id uuid, p_stage_key text, p_current_round text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- BUG-053 (owner decision 2026-07-16): DO NOT write competition_entries here.
  -- Entry stage/current_round are owned exclusively by complete-round
  -- aggregation at round close. This call is now validate + audit only.
  INSERT INTO public.db_audit_logs (table_name, operation, row_id, new_data, changed_by)
  VALUES (
    'competition_entries',
    ...
    jsonb_build_object(
      ...
      'stage_key_asked', p_stage_key,
      'current_round',   p_current_round,
      'write_path',      'judging_write_decision_atomic',
      'entry_write',     'suppressed_per_judge (BUG-053: round-close owns entry stage)',
      'rows_updated',    0
    ),
    auth.uid()
  );
  ...
  RETURN jsonb_build_object(
    ...
    'entry_write_suppressed', true,
    'note', 'entry stage is decided at round close (BUG-053)'
  );
END;
$function$
;
...
BEGIN
  ...
  IF _uid IS NOT NULL THEN
    SELECT count(*) INTO _recent FROM public.client_errors
     WHERE user_id = _uid AND code IS NOT NULL AND created_at > now() - interval '1 hour';
    IF _recent >= 40 THEN RETURN; END IF;
  ELSE
    SELECT count(*) INTO _recent FROM public.client_errors
     WHERE user_id IS NULL AND code IS NOT NULL AND created_at > now() - interval '1 hour';
...
CREATE OR REPLACE FUNCTION public.log_client_error(_kind text, _message text, _detail jsonb DEFAULT NULL::jsonb, _platform text DEFAULT NULL::text, _app_build text DEFAULT NULL::text, _url text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.log_push_outcome(_user_id uuid, _notification_id uuid, _type text, _outcome text, _detail text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.maintain_competition_entry_counts()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
  if (tg_op = 'INSERT') then
    insert into public.competition_entry_counts (competition_id, status, cnt)
      values (new.competition_id, new.status, 1)
      on conflict (competition_id, status)
      do update set cnt = competition_entry_counts.cnt + 1, updated_at = now();
  elsif (tg_op = 'DELETE') then
    update public.competition_entry_counts
      set cnt = greatest(cnt - 1, 0), updated_at = now()
      where competition_id = old.competition_id and status = old.status;
  elsif (tg_op = 'UPDATE') then
...
CREATE OR REPLACE FUNCTION public.media_migrate_post(_post_id uuid, _owner_id uuid, _items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.media_migration_fence_digest(_fence timestamp with time zone)
...
CREATE OR REPLACE FUNCTION public.media_migration_fence_digest_wide(_fence timestamp with time zone)
 RETURNS TABLE(key_set_md5 text, item_count integer, post_count integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.media_write_path_delta(_since timestamp with time zone DEFAULT (now() - '24:00:00'::interval))
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
EXCEPTION WHEN OTHERS THEN
  -- BUG-052: do NOT swallow. Surface a server-log warning and re-raise so the
  -- judge-tag write cannot commit without its mirrored decision.
...
BEGIN
  ...
  WHERE is_active = true
    AND severity IN ('auto_hide', 'high', 'critical')
...
CREATE OR REPLACE FUNCTION public.notif_group_key(_type text, _created_at timestamp with time zone, _id uuid)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
...
CREATE OR REPLACE FUNCTION public.notify_admin_post_report()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.notify_admin_role_application()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  _site_url text := 'https://50mmretina.com';
  _stage_key text;
  -- computed notification payload for this status change
  _kind text;
  _itype text;
  _title text;
  _msg text;
  _tmpl text;
  _data jsonb;
  _is_result boolean := true;   -- result-type outcomes are declaration-gated
  _published boolean;
  _decision_round integer;
  _decision_token text;
  _hold_round integer;
BEGIN
  -- SAFE-MICRO suppression guard (2026-05-08): only judging_write_decision_atomic sets this.
  IF current_setting('app.write_path', true) = 'judging_write_decision_atomic' THEN
    RETURN NEW;
  END IF;
  ...
  -- BUG-109/113: the round a result BELONGS TO (and is released at) is the
  -- decision's source round from the catalog, NOT current_round (which for an
  -- advancer already points at the destination round N+1).
  SELECT round_number, decision_token INTO _decision_round, _decision_token
    FROM public.v3_stage_catalog WHERE stage_key = _stage_key;
  _hold_round := COALESCE(_decision_round, _round_num);

  IF NEW.status ILIKE '%shortlist%' THEN
    _kind := 'entry_shortlisted'; _itype := 'entry_shortlisted'; _tmpl := 'entry-shortlisted';
    _title := 'Your entry was shortlisted';
    _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" has been shortlisted.';
    _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'roundNumber', _round_num, 'stageKey', _stage_key);
  ELSIF NEW.status ILIKE 'round%qualified' OR NEW.status = 'qualified' THEN
    _kind := 'entry_qualified_round'; _itype := 'entry_qualified'; _tmpl := 'entry-qualified-round';
    IF _decision_token = 'accept' THEN
      -- PHASE-2 terminal: "Accepted in Round N" (journey ends at this round)
      _title := 'Accepted in Round ' || COALESCE(_decision_round, _round_num);
      _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" was accepted in Round ' || COALESCE(_decision_round, _round_num) || '.';
      _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'roundNumber', COALESCE(_decision_round, _round_num), 'stageKey', _stage_key, 'terminal', true);
    ELSE
      -- advancing: qualified FOR the next round (source round + 1)
      _title := 'You advanced to the next round';
      _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" qualified for Round ' || COALESCE(_decision_round + 1, _round_num, 2) || '.';
      _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'roundNumber', COALESCE(_decision_round, _round_num), 'nextRound', COALESCE(_decision_round, 1) + 1, 'stageKey', _stage_key);
    END IF;
  ELSIF NEW.status = 'rejected' THEN
    _kind := 'entry_rejected'; _itype := 'entry_rejected'; _tmpl := 'entry-rejected';
    _title := 'Update on your entry';
    _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" did not advance.';
    _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'roundNumber', _round_num, 'stageKey', _stage_key);
  ELSIF NEW.status = 'finalist' THEN
    _kind := 'entry_finalist'; _itype := 'entry_finalist'; _tmpl := 'entry-finalist';
    _title := 'You are a finalist!';
    _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" is a finalist.';
    _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'stageKey', _stage_key);
  ELSIF NEW.status IN ('winner','runner_up','runner_up_1','runner_up_2','honorable_mention','honourable_mention','honorary_mention','special_jury','top_50','top_100') THEN
    _kind := 'entry_award'; _itype := 'competition_winner'; _tmpl := 'entry-winner';
    _title := 'Congratulations on your award!';
    _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" received: ' || NEW.status || '.';
    _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'placement', NEW.status, 'stageKey', _stage_key);
  ELSIF NEW.status = 'approved' THEN
    -- Entry moderation (pre-judging), NOT a round result → notify immediately.
    _kind := 'entry_approved'; _itype := 'entry_approved'; _tmpl := 'entry-shortlisted';
    _title := 'Entry Approved';
    _msg := 'Your entry in "' || COALESCE(_comp_title, 'the competition') || '" has been approved.';
    _data := jsonb_build_object('entryTitle', NEW.title, 'competitionTitle', _comp_title, 'stageKey', _stage_key);
    _is_result := false;
  END IF;

  IF _kind IS NULL THEN
    RETURN NEW;  -- status change we don't notify on
  END IF;

  IF NOT _is_result THEN
    -- immediate (approved / moderation)
    PERFORM public.emit_notification(
      _kind, NEW.id, _round_num, NEW.user_id, _itype, _title, _msg,
      NEW.competition_id, _tmpl, _data, _action_url);
    RETURN NEW;
  END IF;

  -- Result-type: is this entry's round already declared?
  _published := (_hold_round IS NOT NULL) AND EXISTS (
    SELECT 1 FROM public.competition_round_publish crp
    WHERE crp.competition_id = NEW.competition_id
      AND crp.round_number = _hold_round
      AND crp.published_at IS NOT NULL
  );

  IF _published THEN
    -- Round already public (e.g. a post-declaration correction) → emit now.
    PERFORM public.emit_notification(
      _kind, NEW.id, _hold_round, NEW.user_id, _itype, _title, _msg,
      NEW.competition_id, _tmpl, _data, _action_url);
  ELSIF _hold_round IS NOT NULL THEN
    -- Not declared yet → HOLD until publish. Latest pre-publish outcome wins.
    INSERT INTO public.held_result_notifications (
      competition_id, round_number, entity_id, recipient_user_id,
      kind, in_app_type, in_app_title, in_app_message, in_app_reference_id,
      email_template, email_data, action_url)
    VALUES (
      NEW.competition_id, _hold_round, NEW.id, NEW.user_id,
      _kind, _itype, _title, _msg, NEW.competition_id,
      _tmpl, _data, _action_url)
    ON CONFLICT (entity_id, round_number, recipient_user_id) DO UPDATE SET
      kind = EXCLUDED.kind,
      in_app_type = EXCLUDED.in_app_type,
      ...
      created_at = now();
  END IF;
  -- If _round_num IS NULL for a result status (should never happen in practice),
  -- we neither emit nor hold: fail closed, never leak.
...
BEGIN
  PERFORM public.enqueue_post_job(jsonb_build_object(
    ...
    'parent_id',  NEW.parent_id          -- null when not a reply
  ));
  RETURN NEW;
END;
$function$
...
AS $function$
BEGIN
  -- Phase 4: enqueue instead of inserting the notification synchronously.
  PERFORM public.enqueue_post_job(jsonb_build_object(
    'type',           'tag_notification',
    'tag_id',         NEW.id,
    'post_id',        NEW.post_id,
    'tagger_id',      NEW.tagger_id,
    'tagged_user_id', NEW.tagged_user_id
  ));
  RETURN NEW;
END;
$function$
...
CREATE OR REPLACE FUNCTION public.notify_role_application_decision()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  _action_url := _site_url || '/dashboard';
  FOR _user IN
  ...
  LOOP
    PERFORM public.emit_notification(
      'round_published', NEW.competition_id, NEW.round_number, _user.user_id,
      ...
      NEW.competition_id, 'round-published-summary',
      jsonb_build_object('competitionTitle', _comp_title, 'roundNumber', NEW.round_number,
        'entryCount', _user.entry_count),
      _action_url);
  END LOOP;

  -- 3b) Release the granular per-entry outcomes held for THIS round (BUG-019).
  FOR _held IN
    SELECT * FROM public.held_result_notifications
    WHERE competition_id = NEW.competition_id AND round_number = NEW.round_number
  LOOP
    _emit_id := public.emit_notification(
      _held.kind, _held.entity_id, _held.round_number, _held.recipient_user_id,
      _held.in_app_type, _held.in_app_title, _held.in_app_message, _held.in_app_reference_id,
      _held.email_template, _held.email_data, _held.action_url);
    -- BUG-120: only clear the held row once emit actually succeeded.
    IF _emit_id IS NOT NULL THEN
      DELETE FROM public.held_result_notifications WHERE id = _held.id;
    END IF;
  END LOOP;
...
BEGIN
  ...
  LOOP
    ...
    PERFORM public.emit_notification(
      ...
      NEW.competition_id, 'round-published-summary',
      jsonb_build_object('competitionTitle', _comp_title, 'roundNumber', NEW.round_number,
        'entryTitle', _entry.title, 'entryStatus', _entry.status, 'stageKey', _stage_key),
      _action_url);
  END LOOP;

  -- 3b) Release held granular outcomes for this round (BUG-019).
  FOR _held IN
    SELECT * FROM public.held_result_notifications
    WHERE competition_id = NEW.competition_id AND round_number = NEW.round_number
  LOOP
...
AS $function$
DECLARE
  _uid uuid := (event->>'user_id')::uuid;
  _valid boolean := COALESCE((event->>'valid')::boolean, false);
  _row public.auth_login_attempts;
  _lock_seconds integer := 0;
  _remaining integer;
BEGIN
  SELECT * INTO _row FROM public.auth_login_attempts WHERE user_id = _uid FOR UPDATE;

  -- Currently locked -> reject even a correct password until the lock expires.
  IF _row.locked_until IS NOT NULL AND _row.locked_until > now() THEN
    _remaining := CEIL(EXTRACT(EPOCH FROM (_row.locked_until - now())))::integer;
    RETURN jsonb_build_object(
      'decision', 'reject',
      'message', 'Too many failed sign-in attempts. Try again in ' || _remaining || ' seconds.',
  ...
  -- Failed attempt: reset stale counters (1h quiet window), then increment.
  IF _row.user_id IS NULL OR _row.last_attempt < now() - interval '1 hour' THEN
    INSERT INTO public.auth_login_attempts AS a (user_id, failed_count, last_attempt, locked_until)
    VALUES (_uid, 1, now(), NULL)
    ON CONFLICT (user_id) DO UPDATE SET failed_count = 1, last_attempt = now(), locked_until = NULL
    RETURNING * INTO _row;
  ELSE
    UPDATE public.auth_login_attempts
    SET failed_count = failed_count + 1, last_attempt = now()
    WHERE user_id = _uid
    RETURNING * INTO _row;
  END IF;

  _lock_seconds := CASE
    WHEN _row.failed_count >= 10 THEN 900
    WHEN _row.failed_count >= 7  THEN 300
    WHEN _row.failed_count >= 5  THEN 120
    WHEN _row.failed_count >= 3  THEN 30
    ELSE 0
  END;

  IF _lock_seconds > 0 THEN
    UPDATE public.auth_login_attempts
    SET locked_until = now() + make_interval(secs => _lock_seconds)
    WHERE user_id = _uid;
    RETURN jsonb_build_object(
      'decision', 'reject',
      'message', 'Too many failed sign-in attempts. Try again in ' || _lock_seconds || ' seconds.',
      'should_logout_user', false
    );
  END IF;

  -- Let GoTrue return its normal invalid-credentials error.
  RETURN jsonb_build_object('decision', 'continue');
END;
$function$
;
...
CREATE OR REPLACE FUNCTION public.pj_handle_recount_engagement(_msg jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  UPDATE public.posts p SET
    likes_count    = (SELECT COUNT(*) FROM public.post_reactions r WHERE r.post_id = p.id),
...
CREATE OR REPLACE FUNCTION public.pj_handle_tag_notification(_msg jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _tag_id         uuid := (_msg->>'tag_id')::uuid;
  _post_id        uuid := (_msg->>'post_id')::uuid;
  _tagger_id      uuid := (_msg->>'tagger_id')::uuid;
  _tagged_user_id uuid := (_msg->>'tagged_user_id')::uuid;
  _tagger_name    text;
BEGIN
  -- Skip if the tag no longer exists (removed/declined before we ran).
  IF NOT EXISTS (SELECT 1 FROM public.post_tags WHERE id = _tag_id) THEN
    RETURN;
  END IF;

  SELECT COALESCE(full_name, 'Someone') INTO _tagger_name
  FROM public.profiles WHERE id = _tagger_id;

  -- Same title/message as the original sync trigger (migration 20260418111925).
  INSERT INTO public.user_notifications
    (user_id, actor_id, type, title, message, reference_id, dedup_key)
  VALUES (
    _tagged_user_id,
    ...
    'post_tag',
    'New Photo Tag',
    COALESCE(_tagger_name, 'Someone') || ' tagged you in a photo. Approve or decline?',
    _post_id,
    'post_tag:' || _tag_id::text
  )
  ON CONFLICT (dedup_key) WHERE dedup_key IS NOT NULL DO NOTHING;
END;
$function$
...
CREATE OR REPLACE FUNCTION public.post_media_for(_post_ids uuid[])
 RETURNS TABLE(post_id uuid, ord integer, object_path text, width integer, height integer, mime text, bytes bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.post_publish_with_media(_media_ids uuid[], _content text DEFAULT ''::text, _privacy text DEFAULT 'public'::text, _categories text[] DEFAULT '{}'::text[], _indexing_disabled boolean DEFAULT false, _idempotency_key text DEFAULT NULL::text, _thumbnail_urls text[] DEFAULT NULL::text[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- Ownership AND readiness, checked as a set.
  SELECT count(*) INTO _bad
  FROM unnest(_media_ids) AS m(id)
  LEFT JOIN public.media_objects mo ON mo.id = m.id
  ...
  -- The delivery origin, read from settings rather than hardcoded. Fail loud:
  -- a published post carrying half a URL is worse than a refused publish.
  SELECT value->>'public_url' INTO _origin
  FROM public.site_settings WHERE key = 's3_storage_settings';
  ...
  -- image_urls: DERIVED, never supplied, so the legacy array and post_media
  -- cannot disagree about which photograph this is.
  ...
  -- thumbnail_urls: SUPPLIED but CONSTRAINED to the photograph or its -thumb
  -- sibling. Anything else would let a caller point a grid at an image the
  ...
  -- Contiguous ords 0..n-1 by construction (R15).
  INSERT INTO public.post_media (post_id, ord, media_id)
  SELECT _post_id, ord - 1, mid
  FROM unnest(_media_ids) WITH ORDINALITY AS t(mid, ord);
...
CREATE OR REPLACE FUNCTION public.prevent_direct_custom_url_update()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  -- RPCs set this session variable before updating
  IF current_setting('app.allow_custom_url_update', true) = 'true' THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION 'Direct custom_url update is not allowed. Use change_custom_url or clear_custom_url RPC.';
END;
$function$
;
...
CREATE OR REPLACE FUNCTION public.prevent_unfollow_official()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  IF _official IS NOT NULL AND OLD.following_id = _official AND _role = 'authenticated' THEN
    RAISE EXCEPTION 'The official 50mm Retina World account cannot be unfollowed.'
      USING ERRCODE = 'check_violation';
  END IF;
  RETURN OLD;
END;
...
BEGIN
  DELETE FROM public.client_errors WHERE created_at < now() - make_interval(days => GREATEST(7, _days));
...
CREATE OR REPLACE FUNCTION public.prune_old_notifications(_days integer DEFAULT 90, _max_rows integer DEFAULT 5000)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  WITH doomed AS (
    SELECT id FROM public.user_notifications
     WHERE is_read = true AND created_at < now() - make_interval(days => _window)
     ORDER BY created_at LIMIT _cap
  )
  DELETE FROM public.user_notifications n USING doomed d WHERE n.id = d.id;
  GET DIAGNOSTICS _deleted = ROW_COUNT;
...
BEGIN
  ...
  -- Media references. Same guarded shape as the people-tags block below, and
  -- for the same reason: a refused attach must never cost the member the post
  -- they just published. The sub-transaction makes it all-or-nothing, so a
  -- partial carousel is not reachable.
  BEGIN
...
CREATE OR REPLACE FUNCTION public.recompute_entry_public_status(p_entry_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.record_activity_minute(_segment text, _interacted boolean DEFAULT false)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.record_test_agent_run(p_token text, p_run_id text, p_trigger text, p_commit_sha text, p_branch text, p_status text, p_rpc_parity_pass boolean, p_nr_drift_5min integer, p_dual_emit_status text, p_tsc_pass boolean, p_vitest_pass boolean, p_eslint_pass boolean, p_failures jsonb, p_duration_ms integer, p_github_run_url text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.release_award_certs_on_publish()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
BEGIN
  IF NEW.published_at IS NULL OR NEW.round_number <> 4 THEN
    RETURN NEW;
  END IF;
  ...
  UPDATE public.certificates
   ...
   WHERE reference_id = NEW.competition_id
     AND type LIKE 'competition\_%'
...
CREATE OR REPLACE FUNCTION public.request_withdrawal(_amount numeric, _bank_details jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.resolve_custom_url(_url text)
 RETURNS TABLE(user_id uuid, is_current boolean, released_at timestamp with time zone)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.rollup_engagement_daily(_utc_date date DEFAULT NULL::date)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.search_certificates(_name text, _course_title text, _issued_date date)
 RETURNS TABLE(id uuid, title text, description text, type text, issued_at timestamp with time zone, recipient_name text, certificate_id text, verification_token text, is_revoked boolean, revoked_at timestamp with time zone, revoked_reason text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  ...
  WHERE
    -- require at least one real search term (blocks the all-blank bulk dump)
...
BEGIN
  ...
  SELECT token INTO _unsubscribe_token FROM public.email_unsubscribe_tokens WHERE email = lower(_user_email) AND used_at IS NULL LIMIT 1;
  IF _unsubscribe_token IS NULL THEN
    _unsubscribe_token := replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', '');
    INSERT INTO public.email_unsubscribe_tokens (token, email) VALUES (_unsubscribe_token, lower(_user_email)) ON CONFLICT (email) DO NOTHING;
    SELECT token INTO _unsubscribe_token FROM public.email_unsubscribe_tokens WHERE email = lower(_user_email) AND used_at IS NULL LIMIT 1;
  END IF;

  _subject := _notification_type || E' \u2014 50mm Retina World';
  _html := '<!DOCTYPE html><html lang="en" dir="ltr"><head><meta charset="utf-8"/></head><body style="background-color:#ffffff;font-family:-apple-system,BlinkMacSystemFont,''Segoe UI'',Roboto,sans-serif;"><div style="padding:32px 24px;max-width:480px;margin:0 auto;"><h1 style="font-size:20px;font-weight:700;color:#1c2d41;margin:0 0 8px;">Hi ' || public.html_escape(_user_name) || ',</h1><p style="font-size:11px;font-weight:600;color:#0284c7;letter-spacing:0.1em;text-transform:uppercase;margin:0 0 12px;">' || public.html_escape(_notification_type) || '</p><p style="font-size:15px;color:#4b5563;line-height:1.6;margin:0 0 24px;">' || public.html_escape(_notification_message) || '</p><a href="' || _action_url || '" style="display:inline-block;background-color:#0284c7;color:#f0f9ff;padding:12px 28px;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;">View Now</a><hr style="border:none;border-top:1px solid #e5e7eb;margin:28px 0 16px;"/><p style="font-size:12px;color:#9ca3af;line-height:1.5;margin:0;">You are receiving this because you have an account on 50mm Retina World.</p></div></body></html>';
  ...
  PERFORM enqueue_email('transactional_emails', jsonb_build_object(
    'message_id', _message_id, 'to', _user_email,
    'from', '50mm Retina World <no-reply@50mmretina.com>',
    'sender_domain', '50mmretina.com',
    'subject', _subject, 'html', _html, 'text', _plain_text,
    'purpose', 'transactional', 'label', 'notification-alert',
...
CREATE OR REPLACE FUNCTION public.should_test_agent_run(p_trigger text DEFAULT 'schedule'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.soft_void_wallet_transactions(p_txn_ids uuid[], p_reason text, p_batch_id uuid DEFAULT gen_random_uuid())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- BUG-105: authority = admin, OR genuine service_role. A NULL auth.uid() is
  -- ONLY trusted when it is NOT the anon role. Previously ANY null caller was
  -- trusted, so an anon-key caller could void their own wallet debits and have
  -- the balance credited back (self-mint).
  IF _caller IS NOT NULL THEN
    IF NOT has_role(_caller, 'admin'::app_role) THEN
      RAISE EXCEPTION 'soft_void_wallet_transactions: admin/service_role only';
    END IF;
  ELSIF coalesce((current_setting('request.jwt.claims', true))::jsonb ->> 'role', 'service_role') = 'anon' THEN
    RAISE EXCEPTION 'soft_void_wallet_transactions: admin/service_role only';
  END IF;
...
CREATE OR REPLACE FUNCTION public.submit_competition_entry(_competition_id uuid, _title text, _description text, _photos text[], _photo_thumbnails text[], _photo_meta jsonb, _is_ai_generated boolean, _exif_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
AS $function$
BEGIN
  IF NEW.gender IS NOT DISTINCT FROM OLD.gender OR NEW.gender IS NULL THEN RETURN NEW; END IF;
  IF NEW.avatar_url IS NULL OR NEW.avatar_url NOT LIKE '/avatars/fallback/%' THEN RETURN NEW; END IF;
  NEW.avatar_url := '/avatars/fallback/' || CASE WHEN NEW.gender = 'female' THEN 'f' ELSE 'm' END
    || regexp_replace(NEW.avatar_url, '^/avatars/fallback/[mf]([1-5])\.svg$', '\1') || '.svg';
  RETURN NEW;
END; $function$
;
...
CREATE OR REPLACE FUNCTION public.touch_office_staff_updated()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
...
BEGIN
  IF NOT (NEW.status = ANY(_allowed)) THEN
    RAISE EXCEPTION 'Unknown entry status %', NEW.status USING ERRCODE = 'check_violation';
  END IF;
  IF TG_OP = 'INSERT' THEN
    IF NEW.status NOT IN ('draft','submitted','approved') THEN
      RAISE EXCEPTION 'New entries must start as draft, submitted, or approved (got %)', NEW.status USING ERRCODE = 'check_violation';
    END IF;
    RETURN NEW;
  END IF;
  ...
  _ok := CASE OLD.status
    ...
    WHEN 'approved'          THEN NEW.status IN ('submitted','round1_qualified','rejected','hold','shortlisted','needs_review')
    WHEN 'round1_qualified'  THEN NEW.status IN ('submitted','approved','round2_qualified','rejected','hold','shortlisted','needs_review')
    WHEN 'shortlisted'       THEN NEW.status IN ('submitted','approved','round1_qualified','round2_qualified','rejected','hold','needs_review')
    WHEN 'needs_review'      THEN NEW.status IN ('submitted','approved','round1_qualified','shortlisted','rejected','hold')
    WHEN 'round2_qualified'  THEN NEW.status IN ('submitted','round1_qualified','round3_qualified','finalist','rejected','hold','shortlisted')
    WHEN 'round3_qualified'  THEN NEW.status IN ('round2_qualified','finalist','rejected','hold')
    WHEN 'finalist'          THEN NEW.status IN ('round2_qualified','round3_qualified','winner','rejected','hold')
    WHEN 'hold'              THEN NEW.status IN ('approved','submitted','rejected','round1_qualified','round2_qualified','round3_qualified','finalist','shortlisted','needs_review')
    WHEN 'rejected'          THEN NEW.status IN ('hold','submitted','approved','needs_review','shortlisted','round1_qualified')
    WHEN 'winner'            THEN NEW.status IN ('hold','finalist')
    ELSE false
  END;
  IF NOT _ok THEN
...
CREATE OR REPLACE FUNCTION public.validate_post_tag_insert()
 ...
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$ declare _tag_count integer; _was_declined boolean; begin if new.tagger_id <> auth.uid() then raise exception 'You can only create tags as yourself'; end if; select count(*) into _tag_count from public.post_tags where post_id = new.post_id and status in ('pending', 'approved'); if _tag_count >= 20 then raise exception 'Maximum of 20 tags per post reached'; end if; select exists ( select 1 from public.post_tags where post_id = new.post_id and tagger_id = new.tagger_id and tagged_user_id = new.tagged_user_id and status = 'declined' ) into _was_declined; if _was_declined then raise exception 'This user previously declined your tag on this post'; end if; return new; end; $function$
;
...
CREATE OR REPLACE FUNCTION public.verify_certificate(_cert_id text)
 RETURNS TABLE(id uuid, title text, description text, type text, issued_at timestamp with time zone, recipient_name text, certificate_id text, verification_token text, is_revoked boolean, revoked_at timestamp with time zone, revoked_reason text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  ...
  FROM public.certificates c
  LEFT JOIN public.profiles p ON p.id = c.user_id
  WHERE c.id::text = _cert_id
     OR upper(c.certificate_id) = upper(_cert_id)
  LIMIT 1;
$function$
;
...
CREATE OR REPLACE FUNCTION public.verify_staff_id(_id_number text)
 RETURNS TABLE(id_number text, full_name text, designation text, photo_url text, blood_group text, about text, active_from date, expires_on date, job_status text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT s.id_number, s.full_name, s.designation, s.photo_url,
         s.blood_group, s.about, s.active_from, s.expires_on, s.job_status
  FROM public.office_staff s
  WHERE upper(trim(s.id_number)) = upper(trim(coalesce(_id_number, '')))
  LIMIT 1;
$function$
;

CREATE OR REPLACE FUNCTION public.wallet_ledger_apply_v2(p_op text, p_user_id uuid, p_amount numeric, p_idempotency_key text, p_description text DEFAULT NULL::text, p_reference_id text DEFAULT NULL::text, p_source_path text DEFAULT NULL::text, p_dry_run boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.wallet_ledger_v2_diff_snapshot(p_window interval DEFAULT '01:00:00'::interval)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- SECURITY (BUG-001): a non-admin authenticated user must NOT credit (increase)
  -- their own balance via direct RPC. Money-in must come from service_role paths
  -- (verified payments, admin credit, rewards). Self-DEBITS (negative amounts,
  -- e.g. competition fees) remain allowed.
  IF _caller_id IS NOT NULL AND _caller_id = _user_id AND _amount > 0
     AND NOT has_role(_caller_id, 'admin'::app_role) THEN
    RAISE EXCEPTION 'Permission denied: wallet top-up must go through a verified payment';
  END IF;
...
-- Not one of the 9 requested sections, but required for a faithful, complete
-- recreation of the public schema: 10 plain views + 1 materialized view exist
-- in the source database. Placed here, AFTER functions, because two views
-- (entry_public_status, judging_progression_audit) call user-defined
-- functions (has_role(), classify_judging_tag()) that must already exist.
-- entry_final_votes_legacy is placed after entry_final_votes because it
-- selects from it.

CREATE MATERIALIZED VIEW IF NOT EXISTS public.entry_vote_counts AS
 SELECT cv.entry_id,
   ...
   FROM competition_votes cv
     LEFT JOIN ( SELECT admin_vote_adjustments.entry_id,
          ...
          GROUP BY admin_vote_adjustments.entry_id) adj ON adj.entry_id = cv.entry_id
  GROUP BY cv.entry_id, adj.total_adjustment;

CREATE UNIQUE INDEX IF NOT EXISTS idx_entry_vote_counts_entry_id ON public.entry_vote_counts USING btree (entry_id);

CREATE OR REPLACE VIEW public.entry_final_votes AS
 WITH photo_keys AS (
         SELECT competition_votes.entry_id,
            competition_votes.photo_index
           FROM competition_votes
        UNION
         SELECT admin_vote_adjustments.entry_id,
            admin_vote_adjustments.photo_index
           FROM admin_vote_adjustments
        ), real_counts AS (
         SELECT competition_votes.entry_id,
            competition_votes.photo_index,
            count(*)::integer AS real_votes
           FROM competition_votes
          GROUP BY competition_votes.entry_id, competition_votes.photo_index
        ), adj_sums AS (
         SELECT admin_vote_adjustments.entry_id,
            admin_vote_adjustments.photo_index,
            COALESCE(sum(admin_vote_adjustments.adjustment_value), 0::bigint)::integer AS adjustment_total
           FROM admin_vote_adjustments
          GROUP BY admin_vote_adjustments.entry_id, admin_vote_adjustments.photo_index
        )
 SELECT pk.entry_id,
    pk.photo_index,
...
CREATE OR REPLACE VIEW public.entry_final_votes_legacy AS
 SELECT entry_id,
...
CREATE OR REPLACE VIEW public.judge_decisions_owner_safe AS
 SELECT entry_id,
    photo_index,
    decision,
    round_number
...
CREATE OR REPLACE VIEW public.judge_tag_assignments_public_r4 AS
 SELECT id,
    entry_id,
    tag_id,
    photo_index,
...
CREATE OR REPLACE VIEW public.entry_public_status AS
 ...
 SELECT id AS entry_id,
    competition_id,
        ...
        CASE
            WHEN latest_published_round >= 4 THEN ( SELECT array_agg(DISTINCT jt.label ORDER BY jt.label) AS array_agg
               FROM judge_tag_assignments jta
                 JOIN judging_tags jt ON jt.id = jta.tag_id
              WHERE jta.entry_id = base.id AND jt.is_active = true AND jt.is_visible = true AND (4 = ANY (jt.visible_in_round)) AND (jt.label = ANY (ARRAY['Top 50'::text, 'Top 100'::text])))
            ELSE NULL::text[]
...
CREATE OR REPLACE VIEW public.judging_progression_audit AS
 WITH per_photo AS (
         SELECT jta.entry_id,
            jta.photo_index,
         ...
         SELECT per_photo.entry_id,
           ...
           FROM per_photo
          GROUP BY per_photo.entry_id
        ), computed AS (
         SELECT agg.entry_id,
                CASE
                    WHEN agg.any_pass THEN 'qualified'::text
...
UNION ALL
 SELECT 'F2_DECISION_WITHOUT_ALIAS'::text AS finding_code,
   ...
   FROM judge_decisions jd
  WHERE jd.decision IS NOT NULL AND NOT (EXISTS ( SELECT 1
           FROM v3_tag_label_alias a
          WHERE lower(a.alias_label) = lower(jd.decision) AND a.round_number = jd.round_number))
UNION ALL
 SELECT DISTINCT 'F3_TAG_LABEL_NO_ALIAS'::text AS finding_code,
    'judging_tags'::text AS source_table,
    jt.id AS source_row_id,
    NULL::uuid AS entry_id,
    NULL::uuid AS judge_id,
    jta.round_number,
    NULL::integer AS photo_index,
    jt.label AS detail_label,
    NULL::text AS expected_value,
    NULL::text AS actual_value,
   ...
   FROM judge_tag_assignments jta
     JOIN judging_tags jt ON jt.id = jta.tag_id
  WHERE jt.label IS NOT NULL AND NOT (EXISTS ( SELECT 1
           FROM v3_tag_label_alias a
          WHERE lower(a.alias_label) = lower(jt.label) AND a.round_number = jta.round_number))
  GROUP BY jt.id, jt.label, jta.round_number
UNION ALL
 SELECT 'F4_MIRROR_LOG_ERROR'::text AS finding_code,
    ...
    ml.occurred_at
   FROM v3_mirror_log ml
  WHERE ml.action = 'error'::text AND ml.occurred_at >= (now() - '30 days'::interval) AND ml.reviewed_at IS NULL
UNION ALL
 SELECT 'F5_CURRENT_ROUND_INVALID'::text AS finding_code,
   ...
   FROM competition_entries ce
  WHERE ce.current_round IS NOT NULL AND (COALESCE(NULLIF(regexp_replace(ce.current_round, '[^0-9]'::text, ''::text, 'g'::text), ''::text), '0'::text)::integer <> ALL (ARRAY[1, 2, 3, 4]));
...
COMMENT ON TABLE public._v3_quarantine_decisions IS 'Judging v3 cleanup quarantine. Holds judge_decisions rows that violate the v3 catalog (e.g. R2 reject) so they can be reviewed/restored. Populated by Phase 0 Step 0.2.';
COMMENT ON TABLE public.ad_creative_comments IS 'Member comments on a sponsored ad creative. Guarded by the same enforce_comment_blocklist() trigger as post_comments, and flagged into the same comment_reports queue.';
COMMENT ON TABLE public.ad_creative_reactions IS 'Member reactions on a sponsored ad creative. Mirrors post_reactions. Owner request 2026-08-11: ads need like/comment/share like a normal post.';
COMMENT ON TABLE public.ad_creative_shares IS 'One row per member who shared a sponsored ad. Mirrors post_shares.';
COMMENT ON TABLE public.categories IS 'The 46 master photography categories. Single source of truth for slug, display name, icon, strip order and active status. "All" is NOT a row here - it is a client-side system filter that applies no predicate.';
COMMENT ON TABLE public.categories_migration_dropped IS 'Audit of interest labels that could not be mapped to a category slug during the 2026-08-12 taxonomy migration. Expected to be empty. If it is not, those members silently lost an interest and it can be restored by hand from here.';
COMMENT ON TABLE public.client_errors IS 'Client-side failures (post_create, reply, upload, blank_page). Written only by log_client_error(); read only by get_client_error_stats_admin(). 30-day retention.';
COMMENT ON TABLE public.contributor_engagement_daily IS 'Daily rollup of member_activity_minutes. Survives the 40-day prune so lifetime engagement is not lost. minutes_actual is for admin reporting; minutes_credited (capped at 30) is the only figure scoring may ever use. Nothing reads this for scoring during Phase 2a.';
COMMENT ON TABLE public.v3_stage_catalog IS 'Phase R1 Step 1.2 (2026-04-30): Canonical 16-stage catalog. Single source of truth that maps admin-defined judging tag labels to judge_decisions tokens. Consumed by mirror trigger (Step 1.3), eligibility RPC, coverage gate.';
COMMENT ON TABLE public.hashtags IS 'One row per canonical hashtag. Counts are maintained ONLY by trg_posts_sync_hashtags — nothing client-side may write them.';
...
COMMENT ON TABLE public.judging_preflight_log IS 'Append-only audit of every Complete Round preflight check. Lets admins forensically reproduce any UI↔DB drift a judge encountered.';
COMMENT ON TABLE public.member_activity_minutes IS 'One row per member per UTC minute of genuine foreground activity. Written only by record_activity_minute(). surface=internal means the minute touched /admin or /judge and can never earn contributor credit. had_interaction is a diagnostic for detecting over-counting and is never used for scoring.';
COMMENT ON TABLE public.post_drafts IS 'Unpublished posts. Created only when a member presses Save - never automatically - so an abandoned compose session costs nothing. Deleted 7 days after the last edit by prune-post-drafts.';
COMMENT ON TABLE public.post_hashtags IS 'Which post used which hashtag. The primary key makes "the same tag twice in one caption" unrepresentable, so #street #street counts once.';
COMMENT ON TABLE public.post_tags IS 'People tagged in a post. status pending|approved are publicly readable and both render as "with <name>" on the post header (owner ruling 2026-08-10, "show immediately to public"); declined|removed are visible only to the tagger, the tagged member and the post owner, and are never rendered. The tagged member may set status to removed at ANY time from the post menu - "tagged person only can remove my Tag anytime" - which is what makes publishing a pending tag safe.';
COMMENT ON TABLE public.push_delivery_log IS 'One row per notification recording what push_on_notification() decided. "queued" = handed to pg_net, NOT proof of receipt on a handset. Read through get_push_health(). Created 2026-08-01 after push failed silently from launch.';
COMMENT ON TABLE public.v3_mirror_log IS 'Phase R1 Step 1.3 (2026-04-30): forensic log of every mirror_system_tag_to_decision action. Append-only; admins read only.';
...
COMMENT ON TABLE public.wallet_ledger_v2_rows IS 'Phase 1A Step C0: append-only parallel ledger mirror. Written exclusively by wallet_ledger_apply_v2 branch F (live path). Never mutates wallets or wallet_transactions. Admin-only SELECT.';
COMMENT ON TABLE public.wallet_reconciliation_log IS 'Forensic audit trail for wallet_transactions reconciliation (Phase 2.2). Admin-only.';
COMMENT ON COLUMN public.ad_creatives.advertiser_name IS 'Shown in the feed ad header instead of the site name. Empty/NULL = the ad is the platform''s own, and only then is the verified tick drawn.';
COMMENT ON COLUMN public.ad_creatives.advertiser_logo_url IS 'Advertiser''s own logo for the feed ad header. Empty falls back to a letter avatar when advertiser_name is set, or to the site logo when it is not.';
COMMENT ON COLUMN public.categories.slug IS 'Stable storage key. Written into posts.categories and profiles.photography_interests. NEVER rename a slug - add a new row and migrate.';
COMMENT ON COLUMN public.categories.icon_name IS 'A lucide-react export name. The client maps this to a component through a fixed allow-list, because a React component cannot live in a row.';
COMMENT ON COLUMN public.categories.sort_order IS 'Controls the order of the horizontal strip, so which categories fall inside the visible width is an admin decision, not a code change.';
COMMENT ON COLUMN public.certificates.heading IS 'Custom certificates only: the line printed under CERTIFICATE, e.g. "OF APPRECIATION". NULL falls back to OF ACHIEVEMENT.';
COMMENT ON COLUMN public.client_errors.code IS 'Stable catalog code, e.g. POST-2001. Format ^[A-Z]{2,6}-[0-9]{4}$, enforced by log_app_event. Source of truth: src/lib/errorCodes.ts, documented in docs/error-codes.md.';
COMMENT ON COLUMN public.client_errors.event IS 'SCREAMING_SNAKE event name, e.g. STORY_DELETE_MATCHED_NO_ROWS.';
COMMENT ON COLUMN public.client_errors.severity IS 'trace | debug | info | warn | error | fatal.';
...
COMMENT ON COLUMN public.client_errors.correlation_id IS 'Ties every log line of one user action together. Generated per action client-side; NOT a session or device identifier.';
COMMENT ON COLUMN public.comment_reports.ad_comment_id IS 'Set when the report is about an ad_creative_comments row instead of a post_comments row. Exactly one of post_comment_id / ad_comment_id is set.';
...
COMMENT ON COLUMN public.competition_entries.progression_decision IS 'Deterministic SOW-priority aggregated decision per round (shortlisted > qualified/accept > needs_review > reject). Distinct from status (lifecycle).';
...
COMMENT ON COLUMN public.v3_stage_catalog.stage_key IS 'Stable short identifier used by code (e.g. r1_accept, r2_qualified_r3, r4_winner). Never rename.';
COMMENT ON COLUMN public.v3_stage_catalog.tag_label_canonical IS 'Canonical-cased admin tag label. Lookups are case/whitespace-insensitive via lower(trim(...)).';
COMMENT ON COLUMN public.v3_stage_catalog.cert_eligible IS 'Judging v3 / Phase 3.2: TRUE when reaching this stage makes the photo eligible for a certificate. Seeded TRUE for family IN (progression_pass, award); FALSE for rejection, needs_review, progression_fail. Single source of truth used by complete-round and publish-round edge functions.';
...
COMMENT ON COLUMN public.judge_decisions.stage_key IS 'Phase 1 additive (Option B). Canonical v3_stage_catalog.stage_key. NULL allowed during Phase 1; becomes NOT NULL + part of PK in Phase 2.';
COMMENT ON COLUMN public.media_objects.sha256 IS 'Content identity. SERVER-SIDE ONLY — must never appear in a URL, an RPC return type or a client payload. A public CDN plus a content-derived address lets anyone holding candidate bytes probe whether a given member uploaded a given photograph. Enforced by src/__tests__/mediaIdentityContainment.test.ts.';
COMMENT ON COLUMN public.media_objects.state IS 'pending -> verified -> ready, or quarantined. Only `ready` may be referenced by post_media; enforced by trg_post_media_requires_ready, not by convention.';
COMMENT ON COLUMN public.media_objects.quarantine_reason IS 'Why this object was quarantined. NULL unless state = quarantined. Set by media_quarantine(); a quarantine with no reason is not reviewable.';
COMMENT ON COLUMN public.notification_preferences.push_new_posts IS 'Push a phone notification when someone you follow posts. Default FALSE (owner decision 2026-08-01: in-app only). The row is still created and shown in the notification list regardless of this flag.';
COMMENT ON COLUMN public.notification_preferences.push_announcements IS 'Push for platform announcements (new Journal article, new course). Default true: the reach is wanted, the member just needs a way to decline it without switching off push entirely. Read by push_on_notification().';
COMMENT ON COLUMN public.photo_of_the_day.thumbnail_url IS '600px WebP thumbnail for image_url';
COMMENT ON COLUMN public.post_drafts.pending_tags IS 'Array of {taggedUserId, taggedUserName, taggedUserAvatar, photoIndex, xPosition, yPosition}. Coordinates must survive save/resume or people-tags come back attached to nothing.';
COMMENT ON COLUMN public.post_drafts.expiring_at IS 'Set by mark_expiring_post_drafts(). Non-NULL hides the draft from its owner and blocks resume, before any image is deleted.';
COMMENT ON COLUMN public.post_drafts.media_ids IS 'media_objects the composer registered when these photographs were uploaded. Attached to the post by publish_post_draft via post_attach_media. NULL means the draft predates the media write path and publishes legacy-only.';
...
COMMENT ON COLUMN public.posts.indexing_disabled IS 'SOW §5: when true, seo-route-metadata returns noindex,nofollow and sitemap functions exclude this post.';
COMMENT ON COLUMN public.posts.categories IS 'Category SLUGS from public.categories. 1-5 for member posts, empty for system posts and for every post created before 2026-08-12. Empty means the post appears under "All" only.';
COMMENT ON COLUMN public.posts.post_kind IS 'member = a person chose the categories. system = generated by the app (album upload, profile-photo change) with no human choice. SERVER-CONTROLLED: a client can never set this - see enforce_post_categories().';
COMMENT ON COLUMN public.posts.viewer_bucket_stable IS 'Rank input. Refreshed ONLY by refresh_viewer_buckets(), never by the view trigger. A live value reorders the feed mid-session and produces duplicate posts across pages — measured: 2 duplicates in 1532 under 18090 updates.';
COMMENT ON COLUMN public.profiles.photography_interests IS 'Category SLUGS from public.categories (migrated from English labels 2026-08-12). Same vocabulary as posts.categories, so matching a member to posts in their interests is a single && operator.';
COMMENT ON COLUMN public.profiles.indexing_disabled IS 'SOW §5: when true, seo-route-metadata returns noindex,nofollow and sitemap functions exclude this profile.';
COMMENT ON COLUMN public.profiles.last_platform IS 'Where the member last signed in from (app/web). Written by the client alongside last_active_at. Data exists from 2026-08-05 onward only.';
COMMENT ON COLUMN public.scheduled_posts.categories IS 'Category slugs chosen when the post was composed. Copied onto posts.categories at publish time by the publish-scheduled-posts edge function.';
COMMENT ON COLUMN public.scheduled_posts.thumbnail_urls IS 'Thumbnails generated at compose time, index-aligned with image_urls. Carried into posts.thumbnail_urls by publish-scheduled-posts. NULL on rows scheduled before 2026-08-14; the publisher treats NULL as "none available" and backfill-thumbnails repairs the published result.';
COMMENT ON COLUMN public.scheduled_posts.media_ids IS 'As post_drafts.media_ids. Read by publish-scheduled-posts, which runs with no member and therefore cannot re-derive ownership from auth.uid().';
COMMENT ON COLUMN public.v3_mirror_log.write_path IS 'B2.5 — Origin of the write that fired this trigger. Captured from current_setting(''app.write_path''). NULL = caller did not set it (likely direct PostgREST). Edge functions should SET LOCAL app.write_path = ''<fn-name>'' before each write. Measurement-only, no enforcement.';
COMMENT ON COLUMN public.v3_mirror_log.reviewed_at IS 'When set, the row was triaged by an admin and is excluded from forensic drift findings.';
