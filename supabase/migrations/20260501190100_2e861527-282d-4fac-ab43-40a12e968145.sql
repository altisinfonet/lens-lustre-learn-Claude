...
BEGIN
  ...
  SELECT w.entry_id, w.photo_index, w.round_number,
    ...
    CASE
      ...
      -- R3 — distinguish "Accepted in R3" (qualifies in R3 only) from "Shortlisted for Final" (advances to R4)
      WHEN w.round_number = 3 AND w.win_decision IN ('qualified_final','shortlisted_final','shortlisted_for_final','shortlisted for final','qualified','shortlist','shortlisted','finalist') THEN 'r3_qualified_final'
      WHEN w.round_number = 3 AND w.win_decision IN ('accept','accepted')        THEN 'r3_accepted'
      WHEN w.round_number = 3 AND w.win_decision IN ('reject','rejected','skip','not_selected_final') THEN 'r3_not_selected'
      -- R2 — distinguish "Accepted in R2" (qualifies in R2 only) from "Qualified for R3" (advances to R3)
      WHEN w.round_number = 2 AND w.win_decision IN ('qualified_r3','qualified','shortlist','shortlisted') THEN 'r2_qualified_r3'
      WHEN w.round_number = 2 AND w.win_decision IN ('accept','accepted')        THEN 'r2_accepted'
      WHEN w.round_number = 2 AND w.win_decision IN ('skip','reject','rejected','not_selected_r3') THEN 'r2_not_selected'
      -- R1 (unchanged)
