-- ============================================================================
-- Per-photo routing migration
-- Source of truth: judge_decisions per (entry_id, photo_index, round_number)
-- Routing rule: a photo appears in round N (N>=2) iff it has decision='shortlist'
--               in round (N-1) by ANY judge of this competition.
-- ============================================================================

-- ── 1. get_judge_entries_page: per-photo eligibility for R2+ ────────────────
...
RETURNS TABLE(
  id uuid, title text, description text, photos text[], photo_thumbnails text[],
  user_id uuid, status text, created_at timestamp with time zone, competition_id uuid,
  placement text, is_ai_generated boolean, ai_detection_result jsonb, exif_data jsonb,
  view_count integer, current_round text,
  next_cursor_created_at timestamp with time zone, next_cursor_id uuid, has_more boolean
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- ── PER-PHOTO ELIGIBILITY (R2+) ────────────────────────────────────────
  -- An entry is eligible in round N (N>=2) iff at least ONE of its photos
  -- received decision='shortlist' in round (N-1) from any judge of this comp.
  IF _round_number >= 2 THEN
    ...
    IF array_length(_eligible_ids, 1) IS NULL THEN
      RETURN; -- no photos shortlisted from previous round → empty round
    END IF;
  END IF;
...
-- ── 2. apply_decision_to_remaining: per-photo scoping for R2+ ───────────────
...
BEGIN
  ...
  -- ── Build target (entry_id, photo_index) pairs ────────────────────────────
  WITH eligible_pairs AS (
    -- R1: every photo of every entry in the comp.
    -- R2+: only photos that received decision='shortlist' in (round-1).
...
-- ── 3. Per-photo eligibility helper for client filtering ────────────────────
-- Returns (entry_id, photo_index) pairs eligible for a given round in a comp.
-- R1 → all photos; R2+ → only photos shortlisted in the previous round.
CREATE OR REPLACE FUNCTION public.get_round_eligible_photos(
  _competition_id uuid, _round_number integer
)
