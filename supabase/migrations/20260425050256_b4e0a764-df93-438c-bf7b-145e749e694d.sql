...
BEGIN
  ...
  -- Check 1: tag→decision drift (R1 trigger correctness)
  RETURN QUERY
  WITH drift AS (
  ...
  -- Check 2: current_round canonical
  RETURN QUERY
  WITH bad AS (
  ...
  -- Check 4: eligibility consistency — for every active competition with R>=2,
  -- get_round_eligible_photos must return EXACTLY the photos implied by judge_decisions.
  RETURN QUERY
  WITH per_comp AS (
