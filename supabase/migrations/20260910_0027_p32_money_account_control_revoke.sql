...
--
-- FILENAME — RENUMBERED 2026-09-21 by Session A, `_0024` -> `_0027`. This
-- file originated (2026-09-17, then as `_0024`) on a branch cut before the
-- P32 mail-group unit (`20260910_0024_p32mail_delete_email_revoke.sql` and
-- its `_0025`/`_0026` siblings) landed on `main`/`staging` — a genuine
-- concurrent-authorship collision, not an error in either unit; both picked
-- the same "first free" ordinal at the same time from divergent branches.
-- Session A confirmed live on 2026-09-21 that `main`'s highest Phase-1
-- ordinal is `_0026` (the mail-group), so `_0027` is the first free slot as
-- of today. `_0002` (owed a re-cut, `recompute_entry_from_tag_assignments`)
-- and the pre-existing `_0019`/`_0023` ambiguities remain exactly as open as
-- they were — this rename does not touch them. Content below is otherwise
-- byte-identical to the 2026-09-17 original; only this note and the
-- `REVOKE`/`COMMENT` targets (unchanged — they are function names, not
-- filenames) were reviewed for the rename.
-- =============================================================================
