-- Phase 1 retry: DROP+CREATE because RETURNS TABLE shape changed (Postgres 42P13).
-- Existing grantees captured: anon, authenticated, postgres, sandbox_exec,
-- sandbox_exec_isywidnfnjhtydmdfgtk, service_role. All re-granted at end.
...
BEGIN
  ...
  SELECT
    ...
    -- =====================================================================
    -- NEW: canonical Frozen Contract v3 status (Phase 1)
    -- =====================================================================
    CASE
      ...
      -- R3: tightened (F3) to canonical inputs only
      ...
      WHEN c.round_number = 2 AND c.win_decision IN ('accept','accepted')
        THEN 'r2_accepted'
      -- R1: rename to canonical Frozen Contract keys
    ...
    END AS status,
    -- =====================================================================
    -- LEGACY: byte-identical to PRE-Phase-1 emission, dropped in Phase 5
    -- =====================================================================
    CASE
      WHEN c.viewer_role = 'owner' AND c.is_published_v = false
        THEN 'pending_consensus'
...
`status_legacy` for one release (dual-emit). R4 placements still
emitted as winner/finalist here — Phase 3 sibling RPC will own them.
NR is R1-only (F4 defensive + trg_guard_needs_review_round1_only).';

-- Re-grant exactly the access set captured pre-DROP.
