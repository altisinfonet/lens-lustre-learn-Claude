...
-- 1. Guard trigger function: blocks direct custom_url updates unless bypass flag is set
...
DROP TRIGGER IF EXISTS block_custom_url_update ON public.profiles;

CREATE TRIGGER block_custom_url_update
  BEFORE UPDATE ON public.profiles
  FOR EACH ROW
  WHEN (OLD.custom_url IS DISTINCT FROM NEW.custom_url)
  EXECUTE FUNCTION public.prevent_direct_custom_url_update();

-- 3. Update change_custom_url RPC to set bypass flag
...
-- 4. Update clear_custom_url RPC to set bypass flag
