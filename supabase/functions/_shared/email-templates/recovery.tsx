...
export const RecoveryEmail = ({ siteName, siteUrl, confirmationUrl, token, recipient }: RecoveryEmailProps) => {
  ...
  const scannerSafeResetUrl =
    baseUrl && token && recipient
      ? `${baseUrl}/reset-password?recovery_token=${encodeURIComponent(token)}&recovery_email=${encodeURIComponent(recipient)}`
      : confirmationUrl
