...
--   GRANT EXECUTE ON FUNCTION public.emit_notification( text, uuid, integer, uuid, text, text, text, uuid, text...
--
-- --------------------------------------------------------------------------
...
--     \i supabase/rollback/20260910_0025_p32mail_emit_notification_revoke_ROLLBACK.sql
--
...
GRANT EXECUTE ON FUNCTION public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) TO anon, authenticated;

DO $verify$
DECLARE bad int := 0; missing int := 0; o oid;
BEGIN
  o := to_regprocedure('public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text)')::oid;
  IF o IS NULL THEN
    RAISE EXCEPTION 'ROLLBACK POST-CONDITION FAILED -- public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) does not exist on this lane.';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_proc p, aclexplode(p.proacl) a
              WHERE p.oid = o AND a.grantee = 0 AND a.privilege_type = 'EXECUTE') THEN
    bad := bad + 1;
    RAISE WARNING 'PUBLIC holds EXECUTE on public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) after rollback';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p, unnest(p.proacl) a
                  WHERE p.oid = o AND a::text LIKE 'anon=%') THEN
    missing := missing + 1;
    RAISE WARNING 'named anon grant absent on public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) after rollback';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p, unnest(p.proacl) a
                  WHERE p.oid = o AND a::text LIKE 'authenticated=%') THEN
    missing := missing + 1;
    RAISE WARNING 'named authenticated grant absent on public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) after rollback';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p, unnest(p.proacl) a
                  WHERE p.oid = o AND a::text LIKE 'service_role=%') THEN
    missing := missing + 1;
    RAISE WARNING 'named service_role grant absent on public.emit_notification(text, uuid, integer, uuid, text, text, text, uuid, text, jsonb, text) after rollback';
  END IF;
  IF bad > 0 THEN
