-- ═══════════════════════════════════════════════════════════════════════════
-- ROLLBACK for 20260910_0033_p32r7_request_withdrawal_revoke.sql
-- Identical stem, as required. ONE object: public.request_withdrawal(_amount numeric, _bank_details jsonb)
...
-- the one production caller (src/hooks/wallet/useWalletWithdrawals.ts:46) keeps working in
-- both directions.
--
...
DECLARE pub int; an boolean; BEGIN
  SELECT count(*) INTO pub
   ...
   WHERE p.oid = to_regprocedure('public.request_withdrawal(_amount numeric, _bank_details jsonb)')::oid
     AND a.grantee = 0 AND a.privilege_type = 'EXECUTE';
  IF pub > 0 THEN
    RAISE EXCEPTION 'ROLLBACK POST-CONDITION FAILED — PUBLIC holds EXECUTE on public.request_withdrawal(_amount numeric, _bank_details jsonb) after rollback (% entr(y/ies)). This file must never create a PUBLIC grant (UNAPPLIED_0023 hazard). Transaction aborted.', pub;
  END IF;
  SELECT has_function_privilege('anon', to_regprocedure('public.request_withdrawal(_amount numeric, _bank_details jsonb)')::oid, 'EXECUTE') INTO an;
  IF NOT an THEN
    RAISE EXCEPTION 'ROLLBACK POST-CONDITION FAILED — anon does not hold EXECUTE on public.request_withdrawal(_amount numeric, _bank_details jsonb) after the rollback that was supposed to restore it.';
  END IF;
