...
**Created:** 2026-09-02 · **Revision 3 — 2026-09-02** (Revisions 1 and 2 the same day; see §6). Ledger entry for this phase: **REV-22, §34**.
...
|---|---|---|---|---|
| 0.1 | `scripts/db-baseline.mjs` | D1 | **EVIDENCE FILED — awaiting landing** | Two builds exist. The D1 session's — 21 probes, shared `db-lane-guard.mjs`, 28 tests, `d1-baseline.yml` + `d1-guard-check.yml` — is the D-15-compliant author and **is the one landed**; manifest with tree hashes at `claude/d1-phase0/MANIFEST.md`. The Auditor's subagent rebuild is retired as a second instrument; its findings stay as review input. Gate artefact: **BLOCKED on H-4** — the landed instrument has never run against a database. Branch `d1/P0-db-baseline-20260902`, PR pending (courier, D1-DEV-01). |
| 0.2 | Addendum re-measurement | D1 | **EVIDENCE FILED — awaiting landing** | `docs/evidence/d1/baseline/addendum-recheck.md`, one row per figure, on `d1/P0-addendum-recheck-20260902`. Filed 2026-09-02 06:38Z–06:57Z and 11:42Z–11:46Z, read-only against production. Holds exactly: the counter window, realtime decode 50.81 % against 50.9 %, `pg_timezone_names` 2,798 calls at 798.5 ms, the cron purge at 42 runs averaging 6,426 ms, 29 published tables, the same three on REPLICA IDENTITY FULL, 135 MB total, `cron.job_run_details` at 77 MB / 56.7 %, and five of six index sizes byte-identical a day later. Recorded and **not resolved**: presence writes, definer functions 329→332, triggers 149→157, `profiles` dead rows 64.2 %→18.9 %. |
| 0.3 | 1 M-row staging seeder | D1 | **EVIDENCE FILED — awaiting landing** | Seeds `public.posts` only (reservation: `public.posts`). Members, roles and votes need `auth.users` rows — **Owner ruling pending, and P34's scale clause depends on it.** `--ack-enqueue-jobs` refusal added for F-59. Branch `d1/P0-db-seed-staging-20260902`, stacked on 0.1. Three guards built, the third — the cluster's own `system_identifier` — stronger than the kickoff asked for. **All nine `posts` triggers left armed by design**: `rate_limit_posts()` neutralised by backdating, `fan_out_new_post()` throttled by the privacy mix, `detect_duplicate_post()` by distinct deterministic content; `session_replication_role` refused twice over, one of those measured rather than assumed. **Gate not yet met:** the production-ref guard has not been demonstrated failing in a run. |
| 0.4 | `scripts/web-baseline.mjs` | D2 | **IN PROGRESS — CHANGES REQUIRED** | PR #126 against `staging` (branch `altisinfonet-patch-36`, D-19). Reviewed blind by the D2 session: **F-1** wrong unit strings baked into emitted records; **F-3** provenance lost in any worktree. Both confirmed by the Auditor against the PR bytes. Fixes delivered as `claude/d2-phase0/fix-04/` with fail-first evidence; regenerated artefact 291 records, 0 unstamped, `run.git.commit = ef5d4a37`. Awaiting landing and the split (C-50). |
| 0.5 | Web-Vitals harness, report-only | D2 | **IN PROGRESS — CHANGES REQUIRED** | Same PR. **F-2:** the job went green with zero measurements — both guards checked the envelope, neither checked that any route measured. Confirmed. Fix: one step that fails only when no sample measured (`claude/d2-phase0/fix-05/`), proven red/green by running its script. The CI run is the VERIFIED evidence and has not happened yet. Lives under `scripts/` + `.github/workflows/`, not `tools/uishot/**` as 0-D2-02 names; both are D2-owned. |
