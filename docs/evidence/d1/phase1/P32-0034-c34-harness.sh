...
build(){ "$PGBIN/dropdb" -h $H -p $P -U postgres --if-exists $DB >/dev/null 2>&1
         ...
         "$PGBIN/psql" -X -q -h $H -p $P -U postgres -d $DB -v ON_ERROR_STOP=1 -f $ROOT/docs/evidence/d1/phase1/P32-0034-fixture.sql >/dev/null 2>&1; }

# The unit's signatures, straight from the verified TSV.
SIGS=$(awk -F'\t' -v u="$UNIT" '$1==u {printf "%s(%s)\n", $2, $3}' $ROOT/docs/evidence/d1/phase1/P32-0034-signatures-20260925.tsv)
...
anon_calls(){ python3 $ROOT/docs/evidence/d1/phase1/P32-0034-callall.py "$UNIT" "$PGBIN" "$H" "$P" "$DB" "$1"; }
