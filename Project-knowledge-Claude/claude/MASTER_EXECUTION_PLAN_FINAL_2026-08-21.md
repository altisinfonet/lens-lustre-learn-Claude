...
# PART XI — KNOWLEDGE SYNC · 2026-08-21 (POST-PLAN EVENTS, APPEND-ONLY)

**Provenance:** appended 2026-08-21 (evening IST) by the session that ran the Phase-1
closure audit, the Judging Panel safety gate, the Row-12 audit and the Web-staging
pre-flight. **Parts I–X above are unchanged and remain authoritative**; where this Part
records a newer fact, the newer fact carries its evidence document. Percentages are NOT
changed by this Part (Rule 10 below). Repo mirror `docs/MASTER_EXECUTION_PLAN.md` does
not yet carry Part XI (repo write access was unavailable; update it with the next PR).

## XI.1 CURRENT STATE AT A GLANCE

| Area | Status | % | Last verified | Blocker | Next authorized action |
|---|---|---|---|---|---|
| Phase 1 | OPEN — 0/13 certified; rows 8+9 carved out to Judging workstream | none defined | 2026-08-21 (`claude/PHASE1_CLOSURE_AUDIT_2026-08-21.md`, Rev 5) | repo/CI reads (Chrome); owner rulings XI.7 | resume at Row 12 RED TEST once repo reads exist |
| Phase 2 | OPEN | **97** (unchanged) | 2026-08-21 | D-002/D-003 external | "PHASE 2 — CLOSE D-002/D-003" after Cloudflare steps 1–5 |
| Phase 3 | NOT STARTED (spec complete, Part V) | 0 | — | Phase-2 close + D-007/D-008 | none |
| Phase 4 | NOT STARTED | 0 | — | Phase 3 | none |
| Phase 5 | slice LIVE (03:30 UTC daily, `trig_01E5WtgQDsCHhphQZvDoaS9c`); rest NOT STARTED | — | 2026-08-21 | Phase 4 | none |
| Web staging | **NOT READY** — architecture approved (Option B), blocker below | — | 2026-08-21 pre-flight | committed `.env` with production Supabase config | owner approves the §XI.4 isolation fix |
| Android | separate track. Build 1111/v1.2.16 accepted 2026-08-21 09:51 UTC (posts `7eaf0ef8`/`aa98cf72`); fleet rollout pending | — | 2026-08-21 | OWNER: Play rollout (run 32438058103 draft) | owner rollout |
| D-002/D-003 | BLOCKED external; spec+handover complete; negative test still RETRIEVES | (the 3%) | 2026-08-20 | Cloudflare access | owner performs/authorizes §4.3 items 1–5 |
| Judging Panel | **SEPARATE WORKSTREAM — DEFERRED by owner 2026-08-21.** Not PASS, not FAIL, nothing applied | — | safety gate GREEN-with-conditions 2026-08-21 | owner re-authorization only | none in Phases 1–5 |
| Production (web) | live, independent; repo HEAD `c737de9e9e57dd50b6a49a30f503006a6023a3ac` (owner-verified vs GitHub); DB `jtdtehuqtinjxropkkcn` ACTIVE_HEALTHY; 264 posts (263 public / 1 private) at ~12:00 UTC | — | 2026-08-21 | — | read-only unless a gate authorizes |
| Staging (web) | **does not exist yet** — no staging Supabase project, Pages project, or bucket | — | 2026-08-21 (live enumeration) | pre-flight blocker | build only after fix + approval |

## XI.2 PHASE 1 — VERIFIED DELTAS SINCE PARTS III/IX WERE WRITTEN

Full evidence: `claude/PHASE1_CLOSURE_AUDIT_2026-08-21.md` (Revision 5). Deltas only:
- **Row 2 (hashtag gate): production half CONFIRMED live 2026-08-21** — grants, FK
  (convalidated, CASCADE), function ACLs all match BUILD_1106 doc. Needs only repo test
  confirmation to close.
- **Row 4 (duplicate versions): closable** — 0 duplicates in the live ledger and C5's
  verified renames; one HEAD re-check remains.
- **Rows 8 + 9: EXCLUDED from Phase-1 execution** — owner decision 2026-08-21: Judging
  Panel is a separately controlled workstream. Row 9 state: DEFERRED (design + GREEN
  safety gate preserved: `claude/PHASE1_ROW9_ANON_JUDGING_REVOKE_DESIGN_2026-08-21.md`,
  `claude/JUDGING_PANEL_SAFETY_GATE_2026-08-21.md`). Phase-1 exit gate must read "closed
  except rows 8/9, deferred by owner decision".
- **Row 7 (ledger baseline):** Rev-1's "229 vs 32" alarm RESOLVED — the server ledger
  (32 rows, `20260813171159`→`20260820181949`, 0 duplicates) records only
  connector-applied migrations; the ~599 earlier ones ran through the Lovable pipeline
  (C2 §D1.1). Standing direction is C5's 597-row metadata-only baseline (RED files
  `20260429071225`/`20260504132638` excluded); awaiting owner ruling **A2** + a
  file-count recompute at HEAD. ⚠ "229" in Standing Rule 2 almost certainly means the
  **media-object fence** (229 objects), not SQL migrations — reword after repo confirm.
- **Row 12 (realtime/feed/pagination): audit half COMPLETE** —
  `claude/PHASE1_ROW12_FEED_REALTIME_AUDIT_2026-08-21.md`. Live probes P1–P4 all PASS
  (0 privacy leaks incl. adversarial network-ids case; owner sees own private post;
  `is_public` is a STORED GENERATED column — drift impossible). **F12-1: seeded
  deterministic feed ordering is ALREADY LIVE** (`get_broadcast_feed` +
  `feed_rank_score`, per-viewer-per-hour seed) — D-008's cost model and 3R-READ's
  premise need re-derivation. Certification blocked on the row-12 scope ruling (options
  a–d) + which RPC the client uses (repo).
- Rows 6 & 11: evidence STALE — reverify at HEAD (repo/CI). Rows 1, 3, 5: need only
  repo/CI reads. Row 13: OWNER device session.
- Open findings: **E** (live exposure no longer zero — 1 private post `7eaf0ef8` exists;
  three D-003 docs still say "0 non-public"; fix the sentence, and NEVER delete that
  post — it is the Android acceptance evidence), **F** (4th legacy-only tail post
  `5602621c` 2026-08-21 10:30 UTC, user `ba812284`, unattributed; MEDIA-4009/4010
  produced ZERO rows in 48h — G4 not demonstrated for that window until attributed),
  **G** (three `continue-on-error` on debug-APK steps, flagged by C1 2026-08-17, never
  ruled on).

## XI.3 PHASE 2 — STATE CONFIRMED (percentage unchanged: 97)

Write path live and non-vacuous again: 2026-08-21 production shows 5 idempotency-key
posts — 3 real member posts (user `41e124e2`, 04:22/04:32/04:34 UTC, each with
`post_media`) + the 2 Android acceptance posts. Legacy exclusions (D-006, 27+2) and
Class-F repoint stand. D-002/D-003 unchanged: blocked external; closes only on the §4.3
evidence. Remaining Phase-2 blockers: D-002/D-003 only (plus owner Play rollout as the
tail metric, not a %).

## XI.4 WEB STAGING — STATE, ARCHITECTURE, BLOCKER (new workstream, owner-initiated)

Docs: `claude/WEB_STAGING_PRODUCTION_ISOLATION_AUDIT_2026-08-21.md` (architecture,
Option B accepted by owner) and `claude/WEB_STAGING_PREFLIGHT_VERIFICATION_2026-08-21.md`
(pre-flight). **STATUS = NOT READY.** Verified blocker: **the repository commits `.env`
containing production `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY` /
`VITE_SUPABASE_PROJECT_ID`** — Vite auto-loads it, so any build whose env vars are
missing/misspelled silently defaults to production. Also verified: `client.ts` reads
both values purely from `import.meta.env` (no hardcoded fallback; note the real var name
is `VITE_SUPABASE_PUBLISHABLE_KEY`, not ANON_KEY); `index.html` hardcodes production
Supabase preconnect/dns-prefetch; production deploys via **Cloudflare Pages Git
integration** (`web-build.yml` is a build canary with no deploy step); **staging
infrastructure does not exist** (1 Supabase project, no staging Pages project, no
staging bucket).
**Required future fix (approved-in-principle, NOT implemented):** remove committed
`.env` (+ gitignore; local dev uses `.env.local`) · env-aware `index.html`
(`%VITE_SUPABASE_URL%`) · **post-build bundle isolation guard** failing any build whose
`dist/` contains the wrong project ref · staging must FAIL rather than fall back.
**Canonical architecture (owner-accepted):**
`feature → PR → CI → develop → Pages 50mm-staging (staging.50mmretina.com) → automated/
security/mutation/UI tests → owner testing → OWNER APPROVAL → RC frozen (commit SHA,
tree SHA, build id, dist hashes, migration list, edge-function ezbr_sha256 versions,
config version, test results, approval) → merge exact SHA to main → Pages production →
verify deployed output vs RC hashes`. Staging backend = **second free Supabase project**
(schema from a production **schema-only dump — NEVER by replaying the 618 repo
migrations**, per C5/C6) + **R2 bucket `50mm-staging`** behind
`staging-cdn.50mmretina.com` with a bucket-scoped token + staging-only secrets +
synthetic accounts (no PII copies). Cost ₹0 (verified against both vendors' docs).
Production Pages stays Git-connected; Direct-Upload byte-identical promotion is a
separate future workstream. Rollback: Pages deployment Rollback (web) + captured
rollback files (DB).

## XI.5 NON-NEGOTIABLE RULES (owner-issued 2026-08-21; permanent; complement Part II)

1. Evidence beats assumption. 2. No production implementation before staging approval
where staging is technically possible. 3. A staging build must NEVER silently fall back
to production. 4. A human-reported PASS becomes official only with correlated
production/staging evidence. 5. Every approved Release Candidate is identified by
immutable evidence (commit SHA, tree SHA, build id, artifact hashes, test evidence).
6. Failed gate = STOP. 7. Never patch an approved Release Candidate. 8. **Judging Panel
is a completely separate workstream — excluded from Phase 1–5 engineering unless
explicitly re-authorized.** 9. Android and Web release tracks are separate. 10. Audits,
documentation and decision packages never move a completion percentage.

## XI.6 DO NOT CONFUSE THESE (separate workstreams; combine only on explicit authorization)

**Phase 1–5 engineering** (this plan) ≠ **Web staging** (XI.4; precondition work, its own
gates) ≠ **Android release** (separate build/test/rollout track; only its *acceptance
evidence* feeds Phase 2) ≠ **Judging Panel** (deferred security/integrity workstream —
rows 8/9, safety gate, V1 view finding, `judging_write_decision_atomic` guard question)
≠ **D-002/D-003** (external Cloudflare delivery-authorization; the only thing that takes
Phase 2 to 100) ≠ **legacy migration** (CLOSED by decision D-006 — permanent exclusions,
not backlog) ≠ **offline resilience 3R** (Phase 3, NOT started, gated on Phase-2 close +
D-007/D-008).

## XI.7 PENDING OWNER DECISIONS (the full queue, nothing else is waiting on Claude)

1. **GitHub billing before 2026-08-31** — hard deadline; every CI gate dies with it.
2. Play rollout of build 1111. 3. Cloudflare §4.3 steps 1–5 (D-002/D-003).
4. A2 — execute the 597-row ledger baseline (+ write authorization).
5. Row-12 certification scope (options a–d; audit recommends d).
6. Finding F — build attribution for user `ba812284`. 7. Finding E — one-sentence
correction in the three D-003 docs. 8. Finding G — rule on the debug `continue-on-error`
trio. 9. Web-staging isolation fix approval (XI.4), then §19 setup items. 10. D-007 /
D-008 (with F12-1's new evidence). 11. Judging Panel workstream re-authorization
(whenever chosen). 12. Optional: About-screen capture. ⚠ Part X item 5 ("delete WS2
test post 98b2d052") is DONE — already deleted; do not confuse with `7eaf0ef8`.

## XI.8 HOW TO START A NEW CHAT

Before doing any work: 1. Read this Master Plan (Parts I–XI). 2. Read the latest
closure/decision report it references for your workstream. 3. Read that workstream's
latest evidence document. 4. Determine current status from evidence, not memory.
5. Do not repeat completed audits unnecessarily. 6. Do not change scope. 7. Do not
modify production without explicit authorization. 8. Stop at the defined gate.
Practical notes: **this claude.ai project IS readable/writable via the Projects tool —
an earlier session wrongly claimed otherwise; verify tools before declaring them
absent.** The GitHub repo is publicly fetchable file-by-file via
raw.githubusercontent.com pinned to a SHA (read-only). Supabase + Cloudflare MCP
connectors carry the production evidence surface; Chrome connector is required for repo
writes and CI reads. Current HEAD to pin reads against: `c737de9…` (verify it is still
HEAD first).

## XI.9 EVIDENCE INDEX — documents added to this project 2026-08-21

`claude/PHASE1_CLOSURE_AUDIT_2026-08-21.md` (Rev 5) · `claude/JUDGING_PANEL_SAFETY_GATE_2026-08-21.md`
· `claude/PHASE1_ROW9_ANON_JUDGING_REVOKE_DESIGN_2026-08-21.md` ·
`claude/PHASE1_ROW12_FEED_REALTIME_AUDIT_2026-08-21.md` ·
`claude/WEB_STAGING_PRODUCTION_ISOLATION_AUDIT_2026-08-21.md` ·
`claude/WEB_STAGING_PREFLIGHT_VERIFICATION_2026-08-21.md`.
Evidence-integrity rule restated: every completed item carries date, environment,
SHA/version, counts, mutation/CI/production results and known limitations; anything
without evidence is written **NOT VERIFIED** — never converted to PASS. Items this Part
explicitly marks NOT VERIFIED: repo-side tests for Phase-1 rows 1/2/3/5/6/10/11 at HEAD;
friends-positive feed case (vacuous — no friends-posts exist); realtime delivery on
devices; which feed RPC the client uses; `judging_write_decision_atomic` body tail.

**END PART XI. This Part changes documentation only. No engineering change of any kind
was made by the session that wrote it.**
